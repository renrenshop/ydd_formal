...
class Detail_EweiShopV2Page extends PluginMobilePage 
{
	public function main() 
	{
		...
		if (!($cycelbuy_plugin)) 
		{
			show_message($this->lang['_plugin_cycelbuy_core_mobile_goods_detail_234'].'！');
			exit();
		}
		$openid = $_W['openid'];
		$uniacid = $_W['uniacid'];
			...
			foreach ($gid as $ke => $val ) 
			{
				if ($val == $id) 
				{
					...
					foreach ($giftgoods as $k => $val ) 
					{
						...
						$gifttitle = ((!(empty($value['gift'][$k]['title'])) ? $value['gift'][$k]['title'] : $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_252']));
					}
				}
			}
			if (empty($value['gift'])) 
		...
		$goods['unit'] = ((empty($goods['unit']) ? $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_235'] : $goods['unit']));
		...
		if (com('goodscode')) 
		{
			...
			if ($commission_data['codeShare'] == 1) 
			{
				...
				$title = '    ' . $title[0] . "\r\n" . '    ' . $title[1];
				$codedata = array( 'portrait' => array('thumb' => (tomedia($_W['shopset']['shop']['logo']) ? tomedia($_W['shopset']['shop']['logo']) : tomedia($member['avatar'])), 'left' => 40, 'top' => 40, 'width' => 100, 'height' => 100), 'shopname' => array('text' => $_W['shopset']['shop']['name'], 'left' => 160, 'top' => 80, 'size' => 28, 'width' => 360, 'height' => 50, 'color' => '#333'), 'thumb' => array('thumb' => tomedia($goods['thumb']), 'left' => 40, 'top' => 160, 'width' => 560, 'height' => 560), 'qrcode' => array('thumb' => tomedia($qrcode), 'left' => 23, 'top' => 730, 'width' => 220, 'height' => 220), 'title' => array('text' => $title, 'left' => 230, 'top' => 770, 'size' => 24, 'width' => 360, 'height' => 50, 'color' => '#333'), 'price' => array('text' => $price, 'left' => 270, 'top' => 880, 'size' => 30, 'color' => '#f20'), 'desc' => array('text' => $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_253'], 'left' => 210, 'top' => 980, 'size' => 18, 'color' => '#666') );
			}
	...
	public function getCouponsbygood($goodid) 
	{
		...
		if (!(empty($list))) 
		{
			foreach ($list as $key => &$row ) 
			{
				...
				$row['timestr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_236'];
				if (empty($row['timelimit'])) 
				{
					if (!(empty($row['timedays']))) 
					{
						$row['timestr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_237'] . $row['timedays'] . $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_238'];
					}
				}
				else if ($time <= $row['timestart']) 
				{
					$row['timestr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_239'].':' . date('Y-m-d', $row['timestart']) . '-' . date('Y-m-d', $row['timeend']);
				}
				else 
				{
					$row['timestr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_234'].':' . date('Y-m-d', $row['timeend']);
				}
				if ($row['backtype'] == 0) 
				{
					$row['backstr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_242'];
				...
				{
					$row['backstr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_243'];
				...
				else if ($row['backtype'] == 2) 
				{
					...
					{
						$row['backstr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_244'];
						$row['backmoney'] = $row['backredpack'];
						$row['backpre'] = true;
					}
					else if (0 < $row['backmoney']) 
					{
						$row['backstr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_245'];
						$row['backmoney'] = $row['backmoney'];
						$row['backpre'] = true;
					}
					else if (!(empty($row['backcredit']))) 
					{
						$row['backstr'] = $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_246'];
	...
	public function pay($a = array(), $b = array()) 
	{
		...
		{
			show_json(-1, $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_247'] . $coupon['gettypestr']);
		}
		if ($coupon['total'] != -1) 
		{
			if ($coupon['total'] <= 0) 
			{
				show_json(-1, $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_248']);
			}
		}
		if (!($coupon['canget'])) 
		{
			show_json(-1, $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_249'] . $coupon['gettypestr'] . $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_250']);
		}
		if ((0 < $coupon['money']) || (0 < $coupon['credit'])) 
		{
			show_json(-1, $this->lang['_plugin_cycelbuy_core_mobile_goods_detail_251']);
		}
