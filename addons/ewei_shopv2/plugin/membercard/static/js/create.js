define(['core', 'tpl','swiper','biz/order/op'], function (core, tpl,swiper,op) {
    ...
    modal.getCoupon = function () {
        ...
        $(".btn-coupon-receive").bind('click',function () {
            ...
            FoxUI.confirm(lang_js_membercard_js_create_0+'?', lang_js_membercard_js_create_1, function () {
                $(".fui-message-popup .btn-danger").text(lang_js_membercard_js_create_2+'...');
                btn.attr('oldhtml', btn.html()).html(lang_js_membercard_js_create_3+'..').attr('submitting', 1);
                core.json('membercard/coupon/get_month_coupon', {couponid: coupon_id,card_id: modal.params.card_id}, function (ret) {
                    if (ret.status <= 0) {
                        ...
                        $(".fui-message-popup .btn-danger").text(lang_js_membercard_js_create_4);
                        FoxUI.toast.show(ret.result.message);
                        return
                    }else{
                        $('.card-modal').hide();
                        btn.addClass('gary');
                        btn.html(lang_js_membercard_js_create_5);
                        // text = "恭喜您，优惠券到手啦";
                        // button = "立即查看";
    ...
    modal.getPoints = function () {
        ...
        $(".btn-month-points").bind('click',function () {
            ...
            FoxUI.confirm(lang_js_membercard_js_create_6+'?', lang_js_membercard_js_create_7, function () {
                $(".fui-message-popup .btn-danger").text(lang_js_membercard_js_create_8+'...');
                btn.attr('oldhtml', btn.html()).html(lang_js_membercard_js_create_9+'..').attr('submitting', 1);
                core.json('membercard/coupon/get_month_point', {card_id: modal.params.card_id}, function (ret) {
                    if (ret.status <= 0) {
                        btn.html(btn.attr('oldhtml')).removeAttr('oldhtml').removeAttr('submitting');
                        $(".fui-message-popup .btn-danger").text(lang_js_membercard_js_create_10);
                        FoxUI.toast.show(ret.result.message);
                        return
                    }else{
                        btn.addClass('gary');
                        btn.html(lang_js_membercard_js_create_11);
                        // text = "积分领取成功";
                        // button = "立即查看";
