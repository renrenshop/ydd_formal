define(['core', 'tpl', 'swiper', './timer.js'], function(core, tpl, s, timerUtil) {
    ...
    model.init = function(params) {
        ...
        if (params.timecount > 5) {
            model.timeSwiper = new Swiper('.time-container', {
                slidesPerView: 5,
    ...
    model.getCurrentTime = function() {
        return +new Date($.ajax({
            ...
            async: false
        }).getResponseHeader("Date")) / 1000
    };
    model.getGoods = function(timeid) {
        clearInterval(model.timer);
        model.timeid = timeid;

        core.json('seckill/get_goods', {
            taskid: model.taskid,
            roomid: model.roomid,
            timeid: timeid
        }, function(ret) {
    ...
    model.initTimer = function(timeid) {
        ...
        var status = parseInt(slide.data('status')),
            starttime = slide.data('starttime'),
            endtime = slide.data('endtime');
