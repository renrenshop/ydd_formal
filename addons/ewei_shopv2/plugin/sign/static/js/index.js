define(['core', 'tpl'], function(core, tpl) {
	...
	modal.initSign = function() {
		$("#btn-sign").unbind('click').click(function() {
			...
			if (doing) {
				FoxUI.alert(lang_js_sign_static_js_index_0+", "+lang_js_sign_static_js_index_1+'.');
				return
			}
			_this.data('doing', 1);
		...
		if (modal.params.signold) {
			$(document).on('click', "#calendar .day", function() {
				...
				if (doing) {
					FoxUI.alert(lang_js_sign_static_js_index_2+", "+lang_js_sign_static_js_index_3+'.');
					return
				}
				var text = ""+lang_js_sign_static_js_index_4+"" + modal.params.textsignold + ""+lang_js_sign_static_js_index_5+"？";
				if (modal.params.signoldprice > 0) {
					text = modal.params.textsignold + ""+lang_js_sign_static_js_index_6+"" + modal.params.signoldprice + modal.params.signoldtype + "，"+lang_js_sign_static_js_index_7+"" + modal.params.textsignold + ""+lang_js_sign_static_js_index_8+"？"
				}
		...
		$(document).on('click', "#advaward .candraw", function() {
			...
			var doing = _this.data('doing');
			if (doing) {
				FoxUI.alert(lang_js_sign_static_js_index_9+", "+lang_js_sign_static_js_index_10+'.');
				return
			}
			if (!type || !day) {
				modal.getAdvAward();
		...
		core.json('sign/dosign', {
			...
			setTimeout(function() {
				...
				if (ret.status) {
					...
					_this.data('doing', 0);
					if (!date) {
						_this.text(""+lang_js_sign_static_js_index_11+"" + modal.params.textsigned);
