...
class Refund_EweiShopV2Page extends WebPage 
{
										...
										else 
										{
											...
											else 
											{
												...
												if($item["paytype"] == 99){
												    ...
												    $data = array();
												    $data['transactionId'] = $item['transid'];
												    $data['refund']['currencyType'] = 'MYR';
												    $data['refund']['amount'] = $pay_refund_price * 100;
												    $data['reason'] = $refund['reason'];

												    //判断是否部分退款
												    if($pay_refund_price == $item["price"]){
												        $data['refund']['type'] = 'FULL';
                                                    }elseif($item['price'] < $pay_refund_price){
												        $data['refund']['type'] = 'PARTIAL';
                                                    }
                                                    $result = $this->refund_Rmpay($data);
                                                }else{
    ...
    public function refund_Rmpay($data){
        global $_W,$_GPC;

        $order = m('rmpayment')->refund($data);
