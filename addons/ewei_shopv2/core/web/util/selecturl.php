...
class Selecturl_EweiShopV2Page extends WebPage 
{
	...
	public function __construct() 
	{
		...
		$this->all_merch = p('merch')->getMerch();
	}
	public function main() 
	{
        ...
        $merch_list = array();
        foreach ($this->all_merch as $k => $value){
            $qid = pdo_fetchcolumn('SELECT id FROM' . tablename('ewei_shop_quick') . ' WHERE merchid=:id AND uniacid=:uniacid', array(':id' => $value['id'], ':uniacid' => $_W['uniacid']));
            if(!empty($qid)){
                $merch_list[$k] = array(
                    "name" => $value["merchname"],
                    "url" => mobileUrl('quick', array('id' => $qid), true),
                    "url_wxapp" => ""
                );
            }
        }
