	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			$data = array(
                ...
                'qpay_signtype' => intval($_GPC['qpay_signtype']),
                'app_qpay_public_key' => trim($_GPC['app_qpay_public_key']),
                'app_qpay_private_key' => trim($_GPC['app_qpay_private_key']),
			...
			//添加RM支付
			$data['rm'] = array(
			    'Merch_Id'=>$_GPC['rm']['Merch_Id'],
			    'clientId'=>$_GPC['rm']['clientId'],
                'clientSecret'=>$_GPC['rm']['clientSecret']
            );

			$data['rm'] = serialize($data['rm']);
            ...
            if ($_FILES['private_key']['name'])
            {
                $data['private_key'] = $this->upload_cert('private_key');
            }
