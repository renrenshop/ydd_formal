...
class Index_EweiShopV2Page extends WebPage 
{
	...
	public function payset() 
	{
		...
		$payments = pdo_fetchall("SELECT id,title FROM " . tablename("ewei_shop_payment") . " WHERE uniacid=:uniacid and paytype = 2 ", array( ":uniacid" => $_W["uniacid"] ));
    ...
    public function language(){
        ...
        {
            $data =  $_GPC['data'];
            $newdata = $_GPC['newdata'];
            if(!empty($newdata)){
                foreach($newdata as $v){
                    if($v['displayName'] != '' && $v['key'] != ''){
                        $data[] = $v;
                    }
                }
            }
            ...
            m('common')->deleteSysset('language');
            m('common')->updateSysset(array('language' => $data));
            plog('sysset.language.edit', '修改系统设置-语言配置');
            show_json(1);
        }
        $data = m('common')->getSysset('language');
        include $this->template('sysset/language');
    }
}
?>