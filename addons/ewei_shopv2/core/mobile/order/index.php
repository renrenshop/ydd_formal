...
class Index_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function get_list() 
	{
		...
		foreach( $list as &$row ) 
		{
			...
			switch( $row["status"] ) 
			{
				case "-1": $status = $this->lang['lang_module_order_index_3'];
				break;
				case "0": if( $row["paytype"] == 3 ) 
				{
					$status = $this->lang['lang_module_order_index_4'];
				}
				else 
				{
					$status = $this->lang['lang_module_order_index_5'];
				}
				$statuscss = "text-cancel";
				break;
				case "1": if( $row["isverify"] == 1 ) 
				{
					$status = $this->lang['lang_module_order_index_6'];
					if( 0 < $row["verifyendtime"] && $row["verifyendtime"] < time() ) 
					{
						$row["status"] = -1;
						$status = $this->lang['lang_module_order_index_7'];
					}
				}
				else 
				{
					if( empty($row["addressid"]) ) 
					{
						if( !empty($row["ccard"]) ) 
						{
							$status = $this->lang['lang_module_order_index_8'];
						}
						else 
						{
							$status = $this->lang['lang_module_order_index_9'];
						}
					}
					else 
					{
						$status = $this->lang['lang_module_order_index_10'];
						if( 0 < $row["sendtype"] ) 
						{
							$status = $this->lang['lang_module_order_index_11'];
						}
					}
				}
				$statuscss = "text-warning";
				break;
				case "2": $status = $this->lang['lang_module_order_index_12'];
				...
				case "3": if( empty($row["iscomment"]) ) 
				{
					if( $show_status == 5 ) 
					{
						$status = $this->lang['lang_module_order_index_13'];
					}
					else 
					{
						$status = (empty($_W["shopset"]["trade"]["closecomment"]) ? $this->lang['lang_module_order_index_14'] : $this->lang['lang_module_order_index_15']);
					}
				}
				else 
				{
					$status = $this->lang['lang_module_order_index_16'];
				}
			...
			if( 0 < $row["refundstate"] && !empty($row["refundid"]) ) 
			{
				...
				if( !empty($refund) ) 
				{
					$row["statusstr"] = $this->lang['lang_module_order_index_17'] . $r_type[$refund["rtype"]];
				}
			}
			$canrefund = false;
	...
	public function detail() 
	{
		...
		{
			$this->message($this->lang['lang_module_order_index_18'], "", "error");
		}
	...
	public function express() 
	{
		...
		if( empty($order["addressid"]) ) 
		{
			$this->message($this->lang['lang_module_order_index_19']);
		}
		if( 2 > $order["status"] && !(1 <= $order["status"] && 0 < $order["sendtype"]) ) 
		{
			$this->message($this->lang['lang_module_order_index_20']);
		}
		$condition = "";
		if( 0 < $sendtype ) 
		{
