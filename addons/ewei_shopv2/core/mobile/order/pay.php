...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		$pay_type = [
            ...
            array("name"=>'Touch On Go',"type"=>"TNG_MY","img"=>"touch_and_go.png")
        ];
        ...
        foreach ($pay_type as $key => $value){
            ...
            if(!in_array($value['type'] ,$data['pay_type'])){
                unset($pay_type[$key]);
            }
...
//        ppp($data);die;
		...
		if( is_weixin() ) 
		{
			...
			{
			    //替换后类似 SH20190121113246782646 02d
		...
		//查询rm的信息
		if(!empty($set['pay']['weixin_id'])){
		    $sql = 'SELECT * FROM '.tablename('ewei_shop_payment').' WHERE uniacid=:uniacid AND id=:id';
		    $rmpay_config = pdo_fetch($sql,array(':uniacid'=>$_W['uniacid'],':id'=>$set['pay']['weixin_id']));
		    $rm = unserialize($rmpay_config['rm']);

		    if(!empty($rmpay_config) && !empty($rm['clientId']) && !empty($rm['clientSecret']) && !empty($rmpay_config['private_key'])){
		        $rmpay = array('success'=>true);
		    }
		}
	...
	public function payRM(){
	    global $_W,$_GPC;

	    $pay_type = $_GPC['pay_type'];

	    $sql = 'SELECT * FROM '.tablename('ewei_shop_payment').' WHERE uniacid=:uniacid AND id=:id';
	    $rmpay_config = pdo_fetch($sql,array(':uniacid'=>$_W['uniacid'],':id'=>$_GPC['weixin_id']));

        $config = unserialize($rmpay_config['rm']);

        $orderid = $_GPC['id'];

        $error = m('rmpayment')->WebPayment($orderid,$pay_type);

        if(!empty($error)){
            show_json($error['error'],$error['msg']);
            return;
        }
