...
class Index_EweiShopV2Page extends MobilePage 
{
    ...
    public function setLanguage(){
        global $_W,$_GPC;
        $lang_type = strtolower($_GPC['lang_type']);
        if(empty($lang_type)){
            $lang_type = 'en';
        }
        if(!is_dir(EWEI_SHOPV2_LANGUAGE . $lang_type)){
            $lang_type = 'en';
        }
        if(!empty($_W['openid'])){
//            $member = m('member')->getMember($_W['openid']);
            $update = array();
            $update['lang_type'] = $lang_type;
            pdo_update('ewei_shop_member', $update, array('openid' => $_W['openid']));
//            pp($update);
//            $res = pdo_update('mc_members',$update,array('uid'=>$member['uid']));
        }else{
    ...
    public function rmpay_success(){
        global $_GPC,$_W;
        ppp(file_put_contents(dirname(__FILE__).'/text.txt',json_encode($_GPC)));
        ppp(file_put_contents(dirname(__FILE__).'/get.txt',json_encode($_GET)));die;
//        file_put_contents(dirname(__FILE__).'/text.txt',json_encode($_GPC));
