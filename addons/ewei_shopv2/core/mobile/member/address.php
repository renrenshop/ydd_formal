...
//			print_r($address);die;
	...
	public function submit() 
	{
        ...
        // By Wherein
//		if(isset($data['lang_type']) && $data['lang_type']=='ml'){
//            $areas = explode("/", $data["areas"]);
//        }else{
//            $areas = explode(" ", $data["areas"]);
//        }
        unset($data['lang_type']);

		list($data["province"], $data["city"], $data["area"]) = $areas;

        // 省处理
        $is_province_name = pdo_getcolumn('state', ['state_code' => $areas[0]], 'state_name');
        $data['province'] = $is_province_name ? $is_province_name : $data['province'];
	...
	public function get_province(){
	    ...
	    $sql = 'select * from '.tablename('state');
	    $all_province = pdo_fetchall($sql);

	    show_json(1,array('province'=>$all_province));
    ...
    public function get_city(){
        global $_GPC;
        $sql = 'select distinct post_office from '.tablename('postcode').' where state_code=:code';
        $all_city = pdo_fetchall($sql,array(':code'=>$_GPC['state_code']));

        show_json(1,array('city'=>$all_city));

    }

    public function get_area(){
        global $_GPC;
        $sql = 'select area from '.tablename('postcode').' where post_office=:post_office';
        $all_area = pdo_fetchall($sql,array(':post_office'=>$_GPC['post_office']));

        show_json(1,array('area'=>$all_area));
    }
