...
/*
 * The last word is y or n. y means stop var_dump, n means don't stop var_dump.
 * Example1: pp($arr, 'y');
 * Example2: pp(123, 456, 'n');pp('wtf');
 * @author Wherein
 * @return string/arr
 */
function pp() {
...
function ppp(){
    ...
    foreach ($args as $key => $info) {
        ...
        #特殊值
        if($info === null){
            echo '类型：null (gettype：'.gettype($info),')<br>';
            echo '值为null';
            echo '<br/>';
            continue;
        }
        if($info === 0){
            echo '类型：数字0 (gettype：'.gettype($info),')<br>';
            echo $info;
            echo '<br/>';
            continue;
        }
        if($info === false){
            echo '类型：""(gettype：'.gettype($info),')<br>';
            echo '布尔值:false';
            echo '<br/>';
            continue;
        }
        ...
        #类型：数组
        if(is_array($info)){
            echo '类型：数组（gettype：'.gettype($info).')<br/><pre>';
            print_r($info);
            echo '</pre><br/>';
            continue;
            ...
            echo '类型：字符串(gettype：'.gettype($info),')<br>';
            echo $info;
            echo '<br/>';
            continue;
            #类型：浮点数
        }elseif(is_float($info)){
            echo '类型：小数(gettype：'.gettype($info),')<br>';
            echo $info;
            echo '<br/>';
            continue;
            #类型：数字或数字字符串
        }elseif(is_numeric($info)){
            echo '类型：数字(gettype：'.gettype($info),')<br>';
            echo $info;
            echo '<br/>';
            continue;
            #类型：资源
        }elseif(is_resource($info)){
            echo '类型：资源(gettype：'.gettype($info),')<br><pre>';
            var_dump($info);
            echo '</pre><br/>';
            continue;
            ...
            echo '类型：对象(gettype：'.gettype($info),')<br><pre>';
            print_r($info);
            echo '</pre><br/>';
            continue;
        }else{
            echo '类型：未知~(gettype：'.gettype($info),')<br><pre>';
            var_dump($info);
            echo '</pre><br/>';
            continue;
        }
...
if( !function_exists("p") ) 
{
	function p($name = "") 
	{
		...
		if( $return_instance ) 
		{
			...
			//@zong 新增的语言切换
            $model_lang_file = EWEI_SHOPV2_LANGUAGE . $_W['lang_type'] . '/_plugin_' .strtolower($name) . '_core_model.php';


            if(file_exists($model_lang_file)){
                require_once $model_lang_file;
                $_plugins[$name]->loadPluginModelLanguage($lang);
            }
//            echo '<pre>';
//            print_r( $_plugins[$name]);die;
...
if( !function_exists("shop_template_compile") ) 
{
	function shop_template_compile($from, $to, $inmodule = false) 
	{
        ...
        if($_GPC['do'] == 'mobile'){
            //模板语言包
            //echo $from.'<br/>';
            if(strpos($from,'/ewei_shopv2/template/mobile/') !== false){
                $template = m('cache')->getString('template_shop');
                if (empty($template))
                {
                    $template = 'default';
                }
                $lang_file = str_replace(array(IA_ROOT . '/addons/ewei_shopv2/template/mobile/' . $template . '/','.html'),'',$from);
                $lang_file = str_replace('//','/',$lang_file);
                $lang_file = str_replace('/','_',$lang_file);
                $lang_file = EWEI_SHOPV2_LANGUAGE . '\'.$_W[\'lang_type\'].\''. '/_template_mobile_' . $lang_file . '.php';
            }elseif(strpos($from,'/ewei_shopv2/template/account/') !== false){
                $template = $_W['shopset']['wap']['style'];
                if (empty($template))
                {
                    $template = 'default';
                }
                $lang_file = str_replace(array(IA_ROOT . '/addons/ewei_shopv2/template/account/' . $template . '/','.html'),'',$from);
                $lang_file = str_replace('//','/',$lang_file);
                $lang_file = str_replace('/','_',$lang_file);
                $lang_file = EWEI_SHOPV2_LANGUAGE . '\'.$_W[\'lang_type\'].\''. '/_template_account_' . $lang_file . '.php';
            }else{
                $lang_file = str_replace(IA_ROOT . '/addons/ewei_shopv2/','',$from);
                $lang_file = str_replace('.html','',$lang_file);
                $lang_file = str_replace('//','/',$lang_file);
                $lang_file = str_replace('/','_',$lang_file);
                $lang_file = EWEI_SHOPV2_LANGUAGE . '\'.$_W[\'lang_type\'].\''. '/_' . $lang_file . '.php';
            }
            //echo $lang_file;exit;
            //if (file_exists($lang_file)){
            $add_lang_code = "<?php defined('IN_IA') or exit('Access Denied');?>\r\n";
            $add_lang_code .= "<?php if (file_exists('".$lang_file."')){include('".$lang_file."');}?>\r\n";
            $content =  str_replace("<?php defined('IN_IA') or exit('Access Denied');?>",$add_lang_code,$content);
            //}
        }
		...
		foreach( $options["items"] as $item ) 
		{
			...
			else 
			{
				if( $options["type"] == "product" ) 
				{
					...
					{
						$optiontitle = "RM;" . $item["packageprice"];
					}
				...
				else 
				{
					if( $options["type"] == "fullback" ) 
					{
						...
						if( 0 < $item["hasoption"] ) 
						{
							if( $item["type"] == 0 ) 
							{
								$opcontent = "RM" . $item["minallfullbackallprice"] . " ~ RM" . $item["maxallfullbackallprice"];
							}
							else 
							{
						...
						else 
						{
							if( $item["type"] == 0 ) 
							{
								$content = "RM" . $item["minallfullbackallprice"];
							}
							else 
							{
					...
					else 
					{
						if( $options["type"] == "live" ) 
						{
							...
							if( 0 < $item["hasoption"] ) 
							{
								$opcontent = "RM" . $item["minliveprice"] . " ~ RM" . $item["maxliveprice"];
