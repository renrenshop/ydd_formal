...
class MobilePage extends Page 
{
	...
	public function __construct()
	{
		...
		if ($_W['routes'] != 'groups.team.detail') {
...
//            $sql = 'select mm.lang_type from ' . tablename('mc_members') . ' mm left join ' .tablename('ewei_shop_member'). ' m on m.uid = mm.uid where m.openid=:openid';
//            $lang_type = pdo_fetchcolumn($sql,array(':openid'=>$_W['openid']));
            $lang_type = pdo_getcolumn('ewei_shop_member', array('openid' => $_W['openid']), 'lang_type');
            if($lang_type){
                $_W['lang_type'] = $lang_type;
            }
        }

        //获取session里面的默认显示语言
        if(empty($lang_type) && isset($_SESSION['lang_type']) && $_SESSION['lang_type']!=''){
            $_W['lang_type'] = trim($_SESSION['lang_type']);
        }

        $lang_config = array();

        $data = m('common')->getSysset('language');
        if(!empty($data)){
            foreach($data as $k=>$v){
                $v['url'] = mobileUrl('index/setLanguage',array('lang_type'=>$v['key']));
                $lang_config[$v['key']] = $v;
            }
        }
//        pp($lang_config);
        $_W['lang_config'] = $lang_config;

        //加载控制器语言包
        $load_controller_file = str_replace(EWEI_SHOPV2_CORE_MOBILE,'',$_W['load_controller_file']);

        $r = str_replace('/','_',trim($load_controller_file,'/'));

        $lang_file = EWEI_SHOPV2_LANGUAGE . $_W['lang_type'] . '/_module_' .$r;
...
//                    $commission = array('url' => mobileUrl('commission'), 'text' =>  p('commission')->lang['lang_plugin_commission_core_model_3']);
				}
				else 
				{
...
//                    $commission = array('url' => mobileUrl('commission/register'), 'text' => p('commission')->lang['lang_plugin_commission_core_model_4']);
				}
			}
    ...
    public function loadLanguage($lang = array()){
        if(is_array($lang) && !empty($lang)){
            $langTemp = array();
            if(!empty($this->lang)){
                $langTemp = $this->lang;
            }
            $langTemp = array_merge($langTemp,$lang);
            $this->lang = $langTemp;
