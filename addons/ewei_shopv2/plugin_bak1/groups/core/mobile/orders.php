...
class Orders_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function express() 
	{
		...
		if (empty($order['addressid'])) 
		{
			$this->message(''.$this->lang['lang_plugin_groups_core_mobile_orders_0'].'!');
		}
		if ($order['status'] < 2) 
		{
			$this->message(''.$this->lang['lang_plugin_groups_core_mobile_orders_1'].'!');
		}
	...
	public function cancel() 
	{
		...
		try 
		{
			...
			if (empty($order)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_groups_core_mobile_orders_2'].'');
			}
			if ($order['status'] != 0) 
			{
				show_json(0, ''.$this->lang['lang_plugin_groups_core_mobile_orders_3'].'');
			}
	...
	public function delete() 
	{
		...
		if (empty($order)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_groups_core_mobile_orders_4'].'!');
		}
		if (($order['status'] != 3) && ($order['status'] != -1)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_groups_core_mobile_orders_5'].'');
		}
	...
	public function get_list() 
	{
		...
		foreach ($orders as $key => $value ) 
		{
			...
			switch ($value['status']) 
			{
				case '-1': $status = ''.$this->lang['lang_plugin_groups_core_mobile_orders_6'].'';
				break;
				case '0': $status = ''.$this->lang['lang_plugin_groups_core_mobile_orders_7'].'';
				$statuscss = 'text-cancel';
				break;
				case '1': if (($value['is_team'] == 0) || ($value['success'] == 1)) 
				{
					$status = ''.$this->lang['lang_plugin_groups_core_mobile_orders_8'].'';
					$statuscss = 'text-warning';
				}
				else 
				{
					$status = ''.$this->lang['lang_plugin_groups_core_mobile_orders_9'].'';
					$statuscss = 'text-success';
				}
				break;
				case '2': $status = ''.$this->lang['lang_plugin_groups_core_mobile_orders_10'].'';
				$statuscss = 'text-danger';
				break;
				case '3': $status = ''.$this->lang['lang_plugin_groups_core_mobile_orders_11'].'';
				$statuscss = 'text-success';
				break;
			}
		}
	...
	public function confirm() 
	{
		...
		try 
		{
			...
			if ($goods['stock'] <= 0) 
			{
				throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_12'].'');
			}
			$follow = m('user')->followed($openid);
			if (!(empty($goods['followneed'])) && !($follow) && is_weixin()) 
			{
				$followtext = ((empty($goods['followtext']) ? ''.$this->lang['lang_plugin_groups_core_mobile_orders_13'].'~' : $goods['followtext']));
			...
			{
				throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_14'].'');
			}
			...
			{
				throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_15'].'');
			}
			if ($order && ($order['status'] == 1)) 
			{
				throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_16'].'');
			}
			if ($order && ($order['groupnum'] <= $ordernum)) 
			{
				throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_17'].'');
			}
			if (!(empty($teamid))) 
			{
				...
				foreach ($orders as $key => $value ) 
				{
					...
					{
						throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_18'].'');
					}
					if ($orders && ($value['success'] == 1)) 
					{
						throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_19'].'');
					}
				}
				...
				{
					throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_20'].'');
				}
			}
			if ($type == 'groups') 
			{
			...
			if ($_W['ispost']) 
			{
				...
				if ($isverify) 
				{
					...
					{
						throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_21'].'');
					}
				}
				if ((0 < intval($_GPC['aid'])) && !($isverify)) 
				{
					...
					{
						throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_22'].'');
						header('location: ' . mobileUrl('groups/address/post'));
						exit();
					}
					else 
					{
						...
						{
							throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_23'].'');
							header('location: ' . mobileUrl('groups/address/post'));
							exit();
						}
					}
				...
				if (!($order_insert)) 
				{
					throw new Exception(''.$this->lang['lang_plugin_groups_core_mobile_orders_24'].'');
				}
	...
	public function finish() 
	{
		...
		if (empty($order)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_groups_core_mobile_orders_25'].'');
		}
		if ($order['status'] != 2) 
		{
			show_json(0, ''.$this->lang['lang_plugin_groups_core_mobile_orders_26'].'');
		}
