...
class Goods_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_groups_core_mobile_goods_0'].'!', mobileUrl('groups'), 'error');
		}
		...
		if (empty($goods)) 
		{
			$this->message(''.$this->lang['lang_plugin_groups_core_mobile_goods_1'].'!', mobileUrl('groups'), 'error');
		}
	...
	public function openGroups() 
	{
		...
		if (empty($id)) 
		{
			$this->message(''.$this->lang['lang_plugin_groups_core_mobile_goods_2'].'!', mobileUrl('groups'), 'error');
		}
		$goods = pdo_fetch('select * from ' . tablename('ewei_shop_groups_goods') . "\r\n\t\t\t\t\t" . 'where id = :id and status = :status and uniacid = :uniacid and deleted = 0 order by displayorder desc', array(':id' => $id, ':uniacid' => $uniacid, ':status' => 1));
		$goods['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_groups_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $goods['id'], ':uniacid' => $uniacid));
		$goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
		...
		if (empty($goods)) 
		{
			$this->message(''.$this->lang['lang_plugin_groups_core_mobile_goods_3'].'!', mobileUrl('groups'), 'error');
		}
		include $this->template();
	}
	public function fightGroups() 
	{
		...
		if (empty($id)) 
		{
			$this->message(''.$this->lang['lang_plugin_groups_core_mobile_goods_4'].'!', mobileUrl('groups'), 'error');
		}
		$goods = pdo_fetch('select * from ' . tablename('ewei_shop_groups_goods') . "\r\n\t\t\t\t\t" . 'where id = :id and status = :status and uniacid = :uniacid and deleted = 0 order by displayorder desc', array(':id' => $id, ':uniacid' => $uniacid, ':status' => 1));
		$goods['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_groups_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $goods['id'], ':uniacid' => $uniacid));
		$goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
	...
	public function goodsCheck() 
	{
		...
		try 
		{
			...
			if (empty($id)) 
			{
				show_json(0, array('message' => ''.$this->lang['lang_plugin_groups_core_mobile_goods_5'].''));
			}
			$goods = pdo_fetch('select * from ' . tablename('ewei_shop_groups_goods') . "\r\n\t\t\t\t\t" . 'where id = :id and status = :status and uniacid = :uniacid and deleted = 0 order by displayorder desc', array(':id' => $id, ':uniacid' => $uniacid, ':status' => 1));
			if (empty($goods)) 
			{
				show_json(0, array('message' => ''.$this->lang['lang_plugin_groups_core_mobile_goods_6'].''));
			}
			if ($type == 'single') 
			{
				if (empty($goods['single'])) 
				{
					show_json(0, array('message' => ''.$this->lang['lang_plugin_groups_core_mobile_goods_7'].''));
				}
			}
			if (empty($goods['stock'])) 
			{
				show_json(0, array('message' => ''.$this->lang['lang_plugin_groups_core_mobile_goods_8'].'0'.$this->lang['lang_plugin_groups_core_mobile_goods_9'].''));
			}
			show_json(1);
			return NULL;
		}
