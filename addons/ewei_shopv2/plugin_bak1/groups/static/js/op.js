define(['core', 'tpl'], function(core, tpl) {
	...
	modal.init = function(fromDetail) {
		...
		$('.order-cancel select').unbind('change').change(function() {
			...
			FoxUI.confirm(lang_js_groups_static_js_op_0+'?', lang_js_groups_static_js_op_1, function() {
				modal.cancel(orderid, val, true)
			})
		});
		$('.order-delete').unbind('click').click(function() {
			var orderid = $(this).data('orderid');
			FoxUI.confirm(lang_js_groups_static_js_op_2+'?', lang_js_groups_static_js_op_3, function() {
				modal.delete(orderid)
			})
		});
		$('.order-finish').unbind('click').click(function() {
			var orderid = $(this).data('orderid');
			FoxUI.confirm(lang_js_groups_static_js_op_4+'?', lang_js_groups_static_js_op_5, function() {
	...
	modal.verify = function(orderid) {
		...
		core.json('groups/verify/qrcode', {
			...
			if (ret.status == 0) {
				FoxUI.alert(lang_js_groups_static_js_op_6+"，"+lang_js_groups_static_js_op_7+'!');
				return
			}
			var time = +new Date();
