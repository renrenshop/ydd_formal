define(['core', 'tpl'], function(core, tpl) {
	...
	modal.initList = function() {
		...
		$('*[data-toggle=delete]').unbind('click').click(function() {
			...
			FoxUI.confirm(lang_js_groups_static_js_address_0+", "+lang_js_groups_static_js_address_1+' ?', function() {
		...
		$(document).on('click', '[data-toggle=setdefault]', function() {
			...
			core.json('member/address/setdefault', {
				...
				if (ret.status == 1) {
					...
					FoxUI.toast.show(""+lang_js_groups_static_js_address_2+"");
					return
				}
				FoxUI.toast.show(ret.result.message)
			}, true, true)
	...
	modal.initPost = function(params) {
		...
		require(reqParams, function() {
			$('#areas').cityPicker({
				title: lang_js_groups_static_js_address_3,
				...
				onClose: function(self) {
					...
					if (params.new_area && params.address_street) {
						...
						var street = $('<input type="text" id="street"  name="street" data-value="" value="" placeholder=""+lang_js_groups_static_js_address_4+""  class="fui-input" readonly=""/>');
						var parents = $('#street').closest('.fui-cell-info');
						$('#street').remove();
						parents.append(street);
						street.cityPicker({
							title: lang_js_groups_static_js_address_5,
							street: 1,
							data: data
						})
					}
			...
			if (params.new_area && params.address_street) {
				...
				if (datavalue) {
					...
					var data = loadStreetData(city_code, area_code);
					$('#street').cityPicker({
						title: lang_js_groups_static_js_address_6,
						street: 1,
						data: data
					})
				}
		...
		$(document).on('click', '#btn-submit', function() {
			...
			if ($('#realname').isEmpty()) {
				FoxUI.toast.show(""+lang_js_groups_static_js_address_7+"");
				return
			}
			var jingwai = /("+lang_js_groups_static_js_address_8+")+/.test($('#areas').val());
			var taiwan = /("+lang_js_groups_static_js_address_9+")+/.test($('#areas').val());
			var aomen = /("+lang_js_groups_static_js_address_10+")+/.test($('#areas').val());
			var xianggang = /("+lang_js_groups_static_js_address_11+")+/.test($('#areas').val());
			if (jingwai || taiwan || aomen || xianggang) {
				if ($('#mobile').isEmpty()) {
					FoxUI.toast.show(""+lang_js_groups_static_js_address_12+"");
					return
				}
			} else {
				if (!$('#mobile').isMobile()) {
					FoxUI.toast.show(""+lang_js_groups_static_js_address_13+"");
					return
				}
			}
			if ($('#areas').isEmpty()) {
				FoxUI.toast.show(""+lang_js_groups_static_js_address_14+"");
				return
			}
			if ($('#address').isEmpty()) {
				FoxUI.toast.show(""+lang_js_groups_static_js_address_15+"");
				return
			}
			$('#btn-submit').html(lang_js_groups_static_js_address_16+'...').attr('submit', 1);
			...
			core.json('member/address/submit', {
				...
				$('#btn-submit').html(lang_js_groups_static_js_address_17).removeAttr('submit');
				window.editAddressData.id = json.result.addressid;
				if (json.status == 1) {
					FoxUI.toast.show(lang_js_groups_static_js_address_18+'!');
					history.back()
				} else {
