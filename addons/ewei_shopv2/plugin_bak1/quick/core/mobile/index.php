...
class Index_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		if (empty($id)) 
		{
			$this->message(''.$this->lang['lang_plugin_quick_core_mobile_index_0'].'', mobileUrl());
		}
		...
		{
			$this->message(''.$this->lang['lang_plugin_quick_core_mobile_index_1'].'', mobileUrl());
		}
		$datas = htmlspecialchars_decode(base64_decode($item['datas']));
		if (empty($datas)) 
		{
			$this->message(''.$this->lang['lang_plugin_quick_core_mobile_index_2'].'', mobileUrl());
		}
	...
	public function update() 
	{
		...
		if (empty($goodsid)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_quick_core_mobile_index_3'].'('.$this->lang['lang_plugin_quick_core_mobile_index_4'].'id'.$this->lang['lang_plugin_quick_core_mobile_index_5'].')');
		}
		...
		if (empty($goods)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_quick_core_mobile_index_6'].'');
		}
		if (($goods['isverify'] == 2) || ($goods['type'] == 2) || ($goods['type'] == 3) || !(empty($goods['cannotrefund']))) 
		{
			show_json(0, ''.$this->lang['lang_plugin_quick_core_mobile_index_7'].'<br>'.$this->lang['lang_plugin_quick_core_mobile_index_8'].'');
		}
	...
	public function submit() 
	{
		...
		if (empty($list)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_quick_core_mobile_index_9'].'');
		}
		foreach ($list as &$g ) 
		{
			if (empty($g['unit'])) 
			{
				$g['unit'] = ''.$this->lang['lang_plugin_quick_core_mobile_index_10'].'';
			}
			if (($g['status'] != 1) || ($g['goodsdeleted'] == 1)) 
			{
				show_json(0, $g['title'] . '<br/> '.$this->lang['lang_plugin_quick_core_mobile_index_11'].'');
			}
			...
			else 
			{
				...
				if ($g['buylevels'] != '') 
				{
					...
					{
						show_json(0, ''.$this->lang['lang_plugin_quick_core_mobile_index_12'].'<br/>' . $g['title'] . '!');
					}
				}
				if ($g['buygroups'] != '') 
				{
					...
					{
						show_json(0, ''.$this->lang['lang_plugin_quick_core_mobile_index_13'].'<br/>' . $g['title'] . '!');
					}
				}
				if (0 < $g['minbuy']) 
				{
					if ($g['total'] < $g['minbuy']) 
					{
						show_json(0, $g['title'] . '<br/> ' . $g['minbuy'] . $g['unit'] . ''.$this->lang['lang_plugin_quick_core_mobile_index_14'].'!');
					}
				}
				if (0 < $g['maxbuy']) 
				{
					if ($g['maxbuy'] < $g['total']) 
					{
						show_json(0, $g['title'] . '<br/> '.$this->lang['lang_plugin_quick_core_mobile_index_15'].' ' . $g['maxbuy'] . $g['unit'] . '!');
					}
				}
				if (0 < $g['usermaxbuy']) 
				{
					...
					{
						show_json(0, $g['title'] . '<br/> '.$this->lang['lang_plugin_quick_core_mobile_index_16'].' ' . $g['usermaxbuy'] . $g['unit'] . '!');
					}
				}
				if (!(empty($optionid))) 
				{
					...
					if (!(empty($option))) 
					{
						if ($option['stock'] != -1) 
						{
							...
							{
								show_json(-1, $g['title'] . '<br/>' . $option['title'] . ' '.$this->lang['lang_plugin_quick_core_mobile_index_17'].'!');
							}
						}
					}
				}
				else if ($g['stock'] != -1) 
				{
					if (empty($g['stock'])) 
					{
						show_json(0, $g['title'] . '<br/>'.$this->lang['lang_plugin_quick_core_mobile_index_18'].'!');
					}
				}
			}
		}
