define(['core', 'tpl', 'biz/goods/picker', './picker.js', 'biz/plugin/diyform', 'jquery.fly'], function(core, tpl, picker2, picker, diyform) {
	...
	modal.initT2 = function() {
		$(".fui-goods-tab .menu .nav").unbind('click').click(function() {
			...
			if (elm.length > 0) {
				...
				$(".fui-content").stop(true, false).animate({
		...
		$('.fui-content').scroll(function() {
			...
			if ($('.fui-goods-tab').offset().top < h) {
				$('.fui-goods-tab .menu').css({
					position: 'fixed',
					top: h + 'px',
					bottom: '0'
				})
				$('.fui-goods-tab .main').css({
					'margin-left': $('.fui-goods-tab .menu').outerWidth()
				})
			} else {
				$('.fui-goods-tab .menu').css({
					position: 'relative',
		...
		$(".buybtn").unbind('click').click(function() {
			...
			if (goodsid) {
				...
				FoxUI.toast.show(""+lang_js_quick_static_js_mobile_0+"");
			}
		});
	...
	modal.initClick = function() {
		...
		$(document).on('click', ".fui-content .plus", function() {
			...
			if (!item) {
				FoxUI.alert(""+lang_js_quick_static_js_mobile_1+"！"+lang_js_quick_static_js_mobile_2+"")
				return;
			}
			...
			if (item.totalmaxbuy > 0) {
				if (item.totalmaxbuy <= item.num) {
					FoxUI.toast.show(""+lang_js_quick_static_js_mobile_3+"" + item.totalmaxbuy + item.unit);
					return;
				}
			}
			if (item.hasoption == 1 || item.diyformtype > 0 || !item.canAddCart) {
				...
				if (item.canAddCart && item.minbuy > 0 && item.num < item.minbuy) {
					FoxUI.toast.show(""+lang_js_quick_static_js_mobile_4+"/" + item.minbuy + item.unit);
				}
				modal.initSpec(item);
				return;
			}
			if (item.minbuy > 0 && item.num < item.minbuy) {
				FoxUI.toast.show(""+lang_js_quick_static_js_mobile_5+"" + item.minbuy + item.unit);
		...
		$(document).on('click', ".fui-content .minus", function() {
			...
			var item = modal.data[index].data[child];
			if (!item) {
				FoxUI.alert(""+lang_js_quick_static_js_mobile_6+"！"+lang_js_quick_static_js_mobile_7+"")
				return;
			}
			item.num = parseInt(item.num) || 0;
			item.minbuy = parseInt(item.minbuy);
			if (item.hasoption == 1 && item.num > 1) {
				FoxUI.toast.show(""+lang_js_quick_static_js_mobile_8+"");
				return;
			}
			if (item.minbuy > 0 && item.num <= item.minbuy) {
				FoxUI.toast.show(""+lang_js_quick_static_js_mobile_9+"" + item.minbuy + item.unit);
		...
		$(document).on('click', "#quick-cart .plus", function() {
			...
			if (!item) {
				FoxUI.alert(""+lang_js_quick_static_js_mobile_10+"！"+lang_js_quick_static_js_mobile_11+"")
				return;
			}
			item.totalmaxbuy = parseInt(item.totalmaxbuy);
			item.minbuy = parseInt(item.minbuy);
			...
			if (item.minbuy > 0 && item.total < item.minbuy) {
				FoxUI.toast.show(""+lang_js_quick_static_js_mobile_12+"" + item.minbuy + item.unit);
				modal.updateCart($(this), 1, child, parseInt(item.minbuy), 1);
				return;
			}
			if (item.totalmaxbuy > 0) {
				if (item.totalmaxbuy <= item.total) {
					FoxUI.toast.show(""+lang_js_quick_static_js_mobile_13+"" + item.totalmaxbuy + item.unit);
					return;
				}
			}
		...
		$(document).on('click', "#quick-cart .minus", function() {
			...
			var item = modal.cart.list[child];
			if (!item) {
				FoxUI.alert(""+lang_js_quick_static_js_mobile_14+"！"+lang_js_quick_static_js_mobile_15+"")
				return;
			}
			item.minbuy = parseInt(item.minbuy);
			item.total = parseInt(item.total);
			if (item.minbuy > 0 && item.total <= item.minbuy) {
				FoxUI.toast.show(""+lang_js_quick_static_js_mobile_16+"" + item.minbuy + item.unit);
		...
		$("#btn-clear").unbind('click').click(function() {
			...
			FoxUI.confirm(""+lang_js_quick_static_js_mobile_17+"？", function() {
				$("#quick-cart").removeClass('in').addClass('out');
				$("#quick-cart").data('open', 0);
				$(".mask").fadeOut();
				FoxUI.loader.show('loading');
				core.json('quick/clearCart', {
		...
		$("#btn-submit").unbind('click').click(function() {
			...
			_this.html(""+lang_js_quick_static_js_mobile_18+"...");
			var time = 200;
			...
			setTimeout(function() {
				core.json('quick/submit', {
					quickid: modal.fromquick
				}, function(ret) {
					if (ret.status != 1) {
						FoxUI.toast.show(ret.result.message);
						_this.removeAttr("stop").html(""+lang_js_quick_static_js_mobile_19+"");
						return;
					}
					var obj = {
						fromquick: modal.fromquick
					};
	...
	modal.updateCart = function(_this, form, child, num, optionid, diyformdata) {
		...
		if (form == 1) {
			var item = modal.cart.list[child];
			if (!item || !item.id) {
				FoxUI.alert(""+lang_js_quick_static_js_mobile_20+"！"+lang_js_quick_static_js_mobile_21+"")
				return;
			}
			...
			var item = modal.data[index].data[child];
			if (!item || !item.id) {
				FoxUI.alert(""+lang_js_quick_static_js_mobile_22+"！"+lang_js_quick_static_js_mobile_23+"")
				return;
			}
			var oldNum = item.num;
			var goodsid = item.id;
	...
	modal.initSpec = function(item) {
		if (!item || !item.id || modal.child < 0) {
			FoxUI.toast.show(""+lang_js_quick_static_js_mobile_24+"！"+lang_js_quick_static_js_mobile_25+"");
			return;
		}
	...
	modal.initTitle = function() {
		...
		if (item) {
			$("#title").text(item.title || ""+lang_js_quick_static_js_mobile_26+"");
			if (item.desc) {
	...
	modal.initCart = function() {
		...
		if (cart.list.length < 1) {
			...
			$("#cart-price").html("￥0.00").next().html(""+lang_js_quick_static_js_mobile_27+"");
		} else {
			...
			$("#cart-price").html("￥" + modal.cart.totalprice).next().html(""+lang_js_quick_static_js_mobile_28+"");
		}
	};
	modal.getCart = function() {
		core.json('quick/getCart', {
			quickid: modal.fromquick
		}, function(ret) {
			if (ret.status == 0) {
				FoxUI.toast.show(ret.result.message);
	...
	modal.zoom = function(_this) {
		...
		if (_this) {
			...
			dot.fly({
				speed: 1.5,
				start: {
					left: _this.offset().left - 5,
					top: _this.offset().top - 14
				},
				end: {
					left: offset.left + 30,
					top: offset.top - 5
