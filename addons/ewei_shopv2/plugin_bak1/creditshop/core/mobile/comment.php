...
class Comment_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function __construct() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_creditshop_core_mobile_comment_0'].'!', '', 'error');
		}
	}
	public function main() 
	{
		...
		if ($log['goodstype'] == 0) 
		{
			if ($log['status'] != 3) 
			{
				$this->message(''.$this->lang['lang_plugin_creditshop_core_mobile_comment_1'].'!', mobileUrl('creditshop/log/detail', array('id' => $logid)));
			}
		}
		else if ($log['goodstype'] == 1) 
		{
			if ($log['status'] != 3) 
			{
				$this->message(''.$this->lang['lang_plugin_creditshop_core_mobile_comment_2'].'!', mobileUrl('creditshop/log/detail', array('id' => $logid)));
			}
		}
		else if ($log['goodstype'] == 2) 
		{
			if ($log['status'] != 3) 
			{
				$this->message(''.$this->lang['lang_plugin_creditshop_core_mobile_comment_3'].'!', mobileUrl('creditshop/log/detail', array('id' => $logid)));
			}
		}
		else if ($log['goodstype'] == 3) 
		{
			if ($log['status'] != 3) 
			{
				$this->message(''.$this->lang['lang_plugin_creditshop_core_mobile_comment_4'].'!', mobileUrl('creditshop/log/detail', array('id' => $logid)));
			}
		}
		if (2 <= $log['iscomment']) 
		{
			$this->message(''.$this->lang['lang_plugin_creditshop_core_mobile_comment_5'].'!', mobileUrl('creditshop/log/detail', array('id' => $logid)));
		}
	...
	public function submit() 
	{
		...
		if (empty($log)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_comment_6'].'');
		}
		$member = m('member')->getMember($openid);
		$comments = $_GPC['comments'];
		if (!(is_array($comments))) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_comment_7'].'!');
		}
		$trade = m('common')->getSysset('trade');
