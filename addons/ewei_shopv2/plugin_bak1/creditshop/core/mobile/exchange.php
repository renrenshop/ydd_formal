...
class Exchange_EweiShopV2Page extends CreditshopMobilePage 
{
	...
	public function qrcode() 
	{
		...
		if (empty($log)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_0'].'!');
		}
	...
	public function exchange() 
	{
		...
		if (empty($saler)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_1'].'!');
		}
		...
		if (empty($log)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_2'].'!');
		}
		if (empty($log['status'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_3'].'!');
		}
		if (3 <= $log['status']) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_4'].'!');
		}
		$member = m('member')->getMember($log['openid']);
		$goods = $this->model->getGoods($log['goodsid'], $member);
		if (($goods['isendtime'] == 1) && ($goods['endtime'] < time())) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_5'].'!');
		}
		if (empty($goods['id'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_6'].'!');
		}
		if (empty($goods['isverify'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_7'].'!');
		}
		if (!(empty($goods['type']))) 
		{
			if ($log['status'] <= 1) 
			{
				show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_8'].'!');
			}
		}
		if ((0 < $goods['money']) && empty($log['paystatus'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_9'].'!');
		}
		if ((0 < $goods['dispatch']) && empty($log['dispatchstatus'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_10'].'!');
		}
		if (($goods['isendtime'] == 1) && ($goods['endtime'] < $goods['currenttime'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_11'].'!');
		}
		...
		if (!(empty($storeids))) 
		{
			if (!(empty($saler['storeid']))) 
			{
				...
				{
					show_json(0, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_12'].'!');
				}
			}
		}
		$time = time();
		...
		show_json(1, ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_13'].'' . $goods['title'] . ''.$this->lang['lang_plugin_creditshop_core_mobile_exchange_14'].'!');
	}
}
?>