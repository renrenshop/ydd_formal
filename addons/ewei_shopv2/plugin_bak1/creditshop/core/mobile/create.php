...
class Create_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		if (empty($goods)) 
		{
			$this->message(''.$this->lang['lang_plugin_creditshop_core_mobile_create_0'].'!', mobileUrl('creditshop'), 'error');
		}
	...
	public function dispatch() 
	{
		...
		{
			$nodispatch = ''.$this->lang['lang_plugin_creditshop_core_mobile_create_1'].'';
			foreach ($nodispatch_array['title'] as $k => $v ) 
			{
				$nodispatch .= $v . ',';
			}
			$nodispatch = trim($nodispatch, ',');
			$nodispatch .= ''.$this->lang['lang_plugin_creditshop_core_mobile_create_2'].'' . $nodispatch_array['city'];
