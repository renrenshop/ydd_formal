define(['core', 'tpl'], function(core, tpl) {
	...
	modal.init = function(params) {
		...
		if (modal.goods.timestate) {
			$('.fui-timer').timer({
				onEnd: function() {
					$(".fui-navbar .btn").removeClass("btn-danger").addClass("btn-disabled").removeAttr("id").text(""+lang_js_creditshop_static_js_detail_0+"")
				}
			})
		};
		...
		modal.paycheck = function(paytype) {
			...
			if (goods.followneed == '1' && !goods.followed) {
				FoxUI.message.show({
					title: ""+lang_js_creditshop_static_js_detail_1+"",
					icon: 'icon icon-information',
					content: goods.followtext,
					buttons: [{
						text: lang_js_creditshop_static_js_detail_2,
			...
			if (goods.type == 0) {
				if (goods.isverify == 1) {
					if (modal.realname == '') {
						FoxUI.toast.show(lang_js_creditshop_static_js_detail_3+'!');
						return
					}
					if (modal.mobile == '') {
						FoxUI.toast.show(lang_js_creditshop_static_js_detail_4+'!');
						return
					}
					if (modal.storeid == 0) {
						FoxUI.toast.show(lang_js_creditshop_static_js_detail_5+'!');
						return
					}
				} else {
					if (!modal.addressid && goods.goodstype == 0) {
						FoxUI.toast.show(lang_js_creditshop_static_js_detail_6+'!');
						return
					}
				}
				FoxUI.message.show({
					title: ""+lang_js_creditshop_static_js_detail_7+"？",
					icon: 'icon icon-information',
					content: '',
					buttons: [{
						text: lang_js_creditshop_static_js_detail_8,
						extraClass: 'btn-danger',
						onclick: function() {
							modal.pay(goods)
						}
					}, {
						text: lang_js_creditshop_static_js_detail_9,
			...
			if ($(this).parent().hasClass("btn-danger")) {
				if (modal.optionid == 0 && goods.hasoption == 1) {
					...
					core.json('creditshop/detail/option', {
						...
						if (ret.status == 0) {
							FoxUI.toast.show(lang_js_creditshop_static_js_detail_10+'!');
							return
						} else {
		...
		$("#optionid").off("click").on("click", function() {
			...
			core.json('creditshop/detail/option', {
				...
				if (ret.status == 0) {
					FoxUI.toast.show(lang_js_creditshop_static_js_detail_11+'!');
					return
				} else {
					modal.specs = ret.result.specs;
					modal.options = ret.result.options;
	...
	modal.openActionSheet = function(round) {
		FoxUI.actionsheet.show(""+lang_js_creditshop_static_js_detail_12+"", [{
			text: lang_js_creditshop_static_js_detail_13,
			extraClass: 'wechat',
			onclick: function() {
				modal.paycheck('wechat')
			}
		}, {
			text: lang_js_creditshop_static_js_detail_14,
			extraClass: 'alipay',
			onclick: function() {
				modal.paycheck('alipay')
			}
		}, {
			text: lang_js_creditshop_static_js_detail_15,
			extraClass: 'balance',
			onclick: function() {
	...
	modal.chooseSpec = function(obj, callback) {
		...
		if (selected.length == modal.specs.length) {
			...
			$.each(modal.options, function() {
				...
				if (specs == itemids.sort().join('_')) {
					var stock = this.total == '-1' ? lang_js_creditshop_static_js_detail_16 : this.stock;
					$('.total').html(stock);
					if (this.total != '-1' && this.total <= 0) {
						$('.confirmbtn').show().addClass('disabled').html(lang_js_creditshop_static_js_detail_17)
					} else {
						$('.confirmbtn').removeClass('disabled').html(lang_js_creditshop_static_js_detail_18);
		...
		if (modal.option) {
			...
			$('.info-titles').html(lang_js_creditshop_static_js_detail_19+' ' + modal.option.title);
	...
	modal.pay = function() {
		core.json('creditshop/detail/pay', {
			...
			if (result.wechat) {
					...
					function onBridgeReady() {
						WeixinJSBridge.invoke('getBrandWCPayRequest', {
							...
							if (res.err_msg == 'get_brand_wcpay_request:ok') {
								...
								FoxUI.toast.show(lang_js_creditshop_static_js_detail_20)
							} else {
								core.json('creditshop/detail/pay', {
									id: modal.goods.id,
									optionid: modal.optionid,
				...
				if (!alipay.success) {
					FoxUI.toast.show(lang_js_creditshop_static_js_detail_21+'！')
				}
		...
		if (type == 0) {
			core.json('creditshop/detail/lottery', {
				...
				if (result.status == 2) {
					setTimeout(function() {
						FoxUI.message.show({
							title: ""+lang_js_creditshop_static_js_detail_22+"，"+lang_js_creditshop_static_js_detail_23+"!",
							icon: 'icon icon-success',
							content: '',
							buttons: [{
								text: lang_js_creditshop_static_js_detail_24,
								extraClass: 'btn-danger',
								onclick: function() {
					...
					var str = ""+lang_js_creditshop_static_js_detail_25+"";
					if (result.goodstype == 1) {
						str = ""+lang_js_creditshop_static_js_detail_26+""
					} else if (result.goodstype == 2) {
						str = ""+lang_js_creditshop_static_js_detail_27+""
					} else if (result.goodstype == 3) {
						str = ""+lang_js_creditshop_static_js_detail_28+""
					}
					setTimeout(function() {
						FoxUI.message.show({
							title: ""+lang_js_creditshop_static_js_detail_29+"，" + str + ""+lang_js_creditshop_static_js_detail_30+"!",
							icon: 'icon icon-success',
							content: '',
							buttons: [{
								text: lang_js_creditshop_static_js_detail_31,
								extraClass: 'btn-danger',
								onclick: function() {
									location.href = core.getUrl('creditshop/log/detail', {
										id: modal.logid,
			...
			FoxUI.message.show({
				...
				content: lang_js_creditshop_static_js_detail_32+"，"+lang_js_creditshop_static_js_detail_33+'....',
				buttons: []
			});
			setTimeout(function() {
				core.json('creditshop/detail/lottery', {
					...
					if (result.status == 2) {
						FoxUI.message.show({
							title: ""+lang_js_creditshop_static_js_detail_34+"，"+lang_js_creditshop_static_js_detail_35+"!",
							icon: 'icon icon-success',
							content: '',
							buttons: [{
								text: lang_js_creditshop_static_js_detail_36,
								extraClass: 'btn-danger',
								onclick: function() {
									location.href = core.getUrl('creditshop/log/detail', {
										id: modal.logid,
						...
						FoxUI.message.show({
							title: ""+lang_js_creditshop_static_js_detail_37+"，"+lang_js_creditshop_static_js_detail_38+"!",
							icon: 'icon icon-success',
							content: '',
							buttons: [{
								text: lang_js_creditshop_static_js_detail_39,
								extraClass: 'btn-danger',
								onclick: function() {
									location.href = core.getUrl('creditshop/log/detail', {
										id: modal.logid,
						...
						FoxUI.message.show({
							title: ""+lang_js_creditshop_static_js_detail_40+"，"+lang_js_creditshop_static_js_detail_41+"!",
							icon: 'icon icon-wrong',
							content: '',
							buttons: [{
								text: lang_js_creditshop_static_js_detail_42,
								extraClass: 'btn-danger',
								onclick: function() {
									location.reload()
								}
