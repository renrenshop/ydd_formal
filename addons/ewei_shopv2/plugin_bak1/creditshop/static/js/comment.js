	...
	modal.init = function(params) {
		...
		$('.btn-submit').click(function() {
			...
			if (modal.params.iscomment == 0 && modal.params.level < 1) {
				FoxUI.toast.show(lang_js_creditshop_static_js_comment_0);
				return
			}
			if ($('#comment').isEmpty()) {
				FoxUI.toast.show(lang_js_creditshop_static_js_comment_1+'!');
				return
			}
			var default_images = [];
			...
			$(this).html(lang_js_creditshop_static_js_comment_2+'...').attr('stop', 1);
			core.json('creditshop/comment/submit', {
				'logid': modal.params.logid,
				'comments': comments
			}, function(ret) {
				if (ret.status == 1) {
					location.href = core.getUrl('creditshop/log/detail', {
						id: modal.params.logid
					});
					return
				}
				$('.btn-submit').removeAttr('stop').html(lang_js_creditshop_static_js_comment_3);
