...
class PosterProcessor extends PluginProcessor 
{
	...
	{
		global $_W;
		return $obj->respText(''.$this->lang['lang_plugin_poster_core_processor_0'].'!');
	}
	...
	private function responseSubscribe($obj) 
	{
		...
		if (empty($log) && ($openid != $qr['openid'])) 
		{
			...
			{
				$subpaycontent = ''.$this->lang['lang_plugin_poster_core_processor_1'].' [nickname] '.$this->lang['lang_plugin_poster_core_processor_2'].'';
			}
			...
			{
				$recpaycontent = ''.$this->lang['lang_plugin_poster_core_processor_3'].' [nickname] '.$this->lang['lang_plugin_poster_core_processor_4'].'';
			}
			$recpaycontent = str_replace('[nickname]', $member['nickname'], $subpaycontent);
			if (0 < $poster['subcredit']) 
			{
				m('member')->setCredit($openid, 'credit1', $poster['subcredit'], array(0, ''.$this->lang['lang_plugin_poster_core_processor_5'].'+' . $poster['subcredit']));
			}
			...
			if (empty($reward_totle['reccredit_totle']) || (intval($reward_real['reccredit_totle']) <= intval($reward_totle['reccredit_totle']))) 
			{
				...
				{
					m('member')->setCredit($qr['openid'], 'credit1', $poster['reccredit'], array(0, ''.$this->lang['lang_plugin_poster_core_processor_6'].'+' . $poster['reccredit']));
				}
			}
			...
			if (!(empty($poster['subtext']))) 
			{
				...
				{
					m('message')->sendTplNotice($qr['openid'], $poster['templateid'], array( 'first' => array('value' => ''.$this->lang['lang_plugin_poster_core_processor_7'].'', 'color' => '#4a5077'), 'keyword1' => array('value' => ''.$this->lang['lang_plugin_poster_core_processor_8'].'', 'color' => '#4a5077'), 'keyword2' => array('value' => $subtext, 'color' => '#4a5077'), 'remark' => array('value' => "\r\n" . ''.$this->lang['lang_plugin_poster_core_processor_9'].'', 'color' => '#4a5077') ), '');
				}
				else 
				{
			...
			if (!(empty($poster['entrytext']))) 
			{
				...
				if (!(empty($poster['templateid']))) 
				{
					m('message')->sendTplNotice($openid, $poster['templateid'], array( 'first' => array('value' => ''.$this->lang['lang_plugin_poster_core_processor_10'].'', 'color' => '#4a5077'), 'keyword1' => array('value' => ''.$this->lang['lang_plugin_poster_core_processor_11'].'', 'color' => '#4a5077'), 'keyword2' => array('value' => $entrytext, 'color' => '#4a5077'), 'remark' => array('value' => "\r\n" . ''.$this->lang['lang_plugin_poster_core_processor_12'].'', 'color' => '#4a5077') ), '');
				}
				else 
				{
