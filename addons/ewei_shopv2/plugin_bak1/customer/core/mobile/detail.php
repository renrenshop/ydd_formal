	...
	if ($operation == 'display') {
		...
		if ($needpay) {
			...
			if ($useweixin) {
				...
				if (!is_weixin()) {
					show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_detail_0'].'!');
				}


				if (empty($set['pay']['weixin'])) {
					show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_detail_1'].'!');
				}
				...
				$params['title'] = $set['shop']['name'] . ((empty($goods['type']) ? ''.$this->lang['lang_plugin_customer_core_mobile_detail_2'].'' : ''.$this->lang['lang_plugin_customer_core_mobile_detail_3'].'')) . ' '.$this->lang['lang_plugin_customer_core_mobile_detail_4'].':' . $log['logno'];
				load()->model('payment');
				...
				if (!$wechat['success']) {
					show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_detail_5'].'!');
				}
		...
		if (empty($log)) {
			show_json(-1, ''.$this->lang['lang_plugin_customer_core_mobile_detail_6'].'!');
		}


		if (1 <= $log['status']) {
			show_json(-1, ''.$this->lang['lang_plugin_customer_core_mobile_detail_7'].'!');
		}
		...
		if ((0 < $goods['credit']) && empty($log['creditpay'])) {
			m('member')->setCredit($openid, 'credit1', -$goods['credit'], ''.$this->lang['lang_plugin_customer_core_mobile_detail_8'].' ' . $goods['credit']);
			$update['creditpay'] = 1;
		}
		...
		if ((0 < $goods['money']) && empty($log['paystatus'])) {
			if ($goods['paytype'] == 0) {
				m('member')->setCredit($openid, 'credit2', -$goods['money'], ''.$this->lang['lang_plugin_customer_core_mobile_detail_9'].' ' . $goods['credit']);
			}
