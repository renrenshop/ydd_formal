...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		if (empty($log)) {
			show_json(-1, ''.$this->lang['lang_plugin_customer_core_mobile_log_0'].'!');
		}
		...
		if (empty($goods['id'])) {
			show_json(-1, ''.$this->lang['lang_plugin_customer_core_mobile_log_1'].'!');
		}
		...
		if (empty($log)) {
			show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_2'].'!');
		}
		...
		if (empty($goods['id'])) {
			show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_3'].'!');
		}


		if (!empty($goods['isendtime'])) {
			if ($goods['endtime'] < time()) {
				show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_4'].'!');
			}
		...
		if (!empty($log['dispatchstatus'])) {
			show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_5'].'!');
		}
		...
		if (!is_weixin()) {
			show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_6'].'!');
		}


		if (empty($set['pay']['weixin'])) {
			show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_7'].'!');
		}
		...
		$params['title'] = $set['shop']['name'] . ((empty($goods['type']) ? ''.$this->lang['lang_plugin_customer_core_mobile_log_8'].'' : ''.$this->lang['lang_plugin_customer_core_mobile_log_9'].'')) . ' '.$this->lang['lang_plugin_customer_core_mobile_log_10'].':' . $dispatchno;
		load()->model('payment');
		...
		if (!$wechat['success']) {
			show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_11'].'!');
		}
		...
		if (empty($log)) {
			show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_12'].'!');
		}
		...
		if (empty($goods['id'])) {
			show_json(0, ''.$this->lang['lang_plugin_customer_core_mobile_log_13'].'!');
		}
