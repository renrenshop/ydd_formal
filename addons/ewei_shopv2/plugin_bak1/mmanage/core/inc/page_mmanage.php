...
class MmanageMobilePage extends PluginMobilePage 
{
	public function __construct($_com = '', $_init = false) 
	{
		...
		if (empty($_GPC['i'])) 
		{
			$this->message(''.$this->lang['lang_plugin_mmanage_core_inc_page_mmanage_0'].'');
		}
		...
		{
			$this->message(''.$this->lang['lang_plugin_mmanage_core_inc_page_mmanage_1'].'', mobileUrl());
		}
		$this->checkLogin();
		$this->setShare();
	}
	...
	protected function setShare() 
	{
		...
		$set = $this->set;
		$GLOBALS['_W']['shopshare'] = array('title' => (!(empty($set['title'])) ? $set['title'] : $shopset['name'] . ''.$this->lang['lang_plugin_mmanage_core_inc_page_mmanage_2'].''), 'imgUrl' => (!(empty($set['thumb'])) ? tomedia($set['thumb']) : tomedia($shopset['logo'])), 'desc' => (!(empty($set['desc'])) ? $set['desc'] : $shopset['description']), 'link' => mobileUrl('mmanage', array(), true));
	}
}
?>