...
class Index_EweiShopV2Page extends MmanageMobilePage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($realname)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_set_index_0'].'');
			}
			if (empty($realname)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_set_index_1'].'');
			}
			if (!(empty($password)) || !(empty($password2))) 
			{
				if (empty($password)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_set_index_2'].'');
				}
				if (empty($password2)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_set_index_3'].'');
				}
				if ($password != $password2) 
				{
					show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_set_index_4'].'');
				}
				$changepass = true;
			}
			load()->model('user');
