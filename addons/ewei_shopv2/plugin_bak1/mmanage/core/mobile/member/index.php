...
class Index_EweiShopV2Page extends MmanageMobilePage 
{
	public function main() 
	{
		...
		if (!(cv('member'))) 
		{
			$this->message(''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_0'].'');
		}
	...
	public function getlist() 
	{
		...
		foreach ($list as &$row ) 
		{
			...
			$row['levelname'] = ((empty($row['levelname']) ? ((empty($shop['levelname']) ? ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_1'].'' : $shop['levelname'])) : $row['levelname']));
			...
			$row['groupname'] = ((empty($row['groupname']) ? ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_2'].'' : $row['groupname']));
		}
		unset($row);
	...
	public function detail() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_3'].'');
		}
		$id = intval($_GPC['id']);
		...
		if (empty($member)) 
		{
			if ($_W['isajax']) 
			{
				show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_4'].'');
			}
			else 
			{
				$this->message(''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_5'].'');
			}
		}
		...
		if ($_W['ispost']) 
		{
			...
			else 
			{
				if (empty($member['mobileverify']) || empty($member['mobile'])) 
				{
					...
					{
						show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_6'].'');
					}
					...
					if (!(empty($m)) && ($m['id'] != $id)) 
					{
						show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_7'].'!(uid:' . $m['id'] . ')');
					}
					if (!(empty($data['mobileverify'])) && empty($password)) 
					{
						show_json(0, ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_8'].'');
					}
					if (!(empty($password))) 
					{
						$salt = $member['salt'];
			...
			if ($plugin_commission && cv('commission.agent.edit')) 
			{
				...
				if (cv('commission.agent.check')) 
				{
					if (empty($member['status']) && !(empty($data['status']))) 
					{
						...
						plog('commission.agent.check', ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_9'].' <br/>'.$this->lang['lang_plugin_mmanage_core_mobile_member_index_10'].':  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
				...
				plog('commission.agent.edit', ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_11'].' <br/>'.$this->lang['lang_plugin_mmanage_core_mobile_member_index_12'].':  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			pdo_update('ewei_shop_member', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			plog('member.list.edit', ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_13'].'  ID: ' . $member['id'] . ' <br/> '.$this->lang['lang_plugin_mmanage_core_mobile_member_index_14'].':  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			show_json(1);
		}
		...
		$member['last_ordertime'] = ((empty($member['last_ordertime']) ? ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_15'].'' : date('Y-m-d H:i:s', $member['last_ordertime'])));
		if (!(empty($member['agentid']))) 
		{
		...
		if ($plugin_commission) 
		{
			...
			$comlevel_title = ((empty($plugin_com_set['levelname']) ? ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_16'].'' : $plugin_com_set['levelname']));
		...
		$shop = $_W['shopset']['shop'];
		$level_title = ((empty($shop['levelname']) ? ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_17'].'' : $shop['levelname']));
		...
		$group_title = ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_18'].'';
		...
		$goods_title = ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_19'].'';
		if ($member['agentselectgoods'] == 1) 
		{
			$goods_title = ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_20'].'';
		}
		else if ($member['agentselectgoods'] == 2) 
		{
			$goods_title = ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_21'].'';
		}
	...
	public function setblack() 
	{
		...
		foreach ($members as $member ) 
		{
			if (!(empty($black))) 
			{
				...
				plog('member.list.edit', ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_22'].' <br/>'.$this->lang['lang_plugin_mmanage_core_mobile_member_index_23'].':  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else 
			{
				pdo_update('ewei_shop_member', array('isblack' => 0), array('id' => $member['id']));
				plog('member.list.edit', ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_24'].' <br/>'.$this->lang['lang_plugin_mmanage_core_mobile_member_index_25'].':  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
		show_json(1);
	}
	public function delete() 
	{
		...
		foreach ($members as $member ) 
		{
			...
			plog('member.list.delete', ''.$this->lang['lang_plugin_mmanage_core_mobile_member_index_26'].'  ID: ' . $member['id'] . ' <br/>'.$this->lang['lang_plugin_mmanage_core_mobile_member_index_27'].': ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		}
