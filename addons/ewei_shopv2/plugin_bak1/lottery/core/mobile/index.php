...
class index_EweiShopV2Page extends PluginMobilePage 
{
	...
	public function myrewardpage() 
	{
		...
		foreach ($mylog as $key => $value ) 
		{
			...
			{
				$mylog[$key]['title'] = ''.$this->lang['lang_plugin_lottery_core_mobile_index_0'].':' . $lottery_data['credit'];
			...
			{
				$mylog[$key]['title'] = ''.$this->lang['lang_plugin_lottery_core_mobile_index_1'].':' . $lottery_data['money']['num'];
				$mylog[$key]['rewarded'] = 1;
				$mylog[$key]['icon'] = '../addons/ewei_shopv2/plugin/lottery/static/images/jiangjin.png';
			}
			else if (isset($lottery_data['bribery'])) 
			{
				$mylog[$key]['title'] = ''.$this->lang['lang_plugin_lottery_core_mobile_index_2'].':' . $lottery_data['bribery']['num'];
				$mylog[$key]['rewarded'] = 1;
			...
			else if (isset($lottery_data['goods'])) 
			{
				...
				$mylog[$key]['title'] = ''.$this->lang['lang_plugin_lottery_core_mobile_index_3'].':' . $goods['title'];
			...
			else if (isset($lottery_data['coupon'])) 
			{
				...
				$mylog[$key]['title'] = ''.$this->lang['lang_plugin_lottery_core_mobile_index_4'].':' . $coupon['couponname'];
				$mylog[$key]['rewarded'] = 1;
	...
	public function getreward() 
	{
		...
		if (empty($temreward)) 
		{
			$data = array('status' => 0, 'info' => ''.$this->lang['lang_plugin_lottery_core_mobile_index_5'].','.$this->lang['lang_plugin_lottery_core_mobile_index_6'].'!');
			echo json_encode($data);
			exit();
		}
		...
		{
			$is_reward = 0;
			$reward_info = ''.$this->lang['lang_plugin_lottery_core_mobile_index_7'].','.$this->lang['lang_plugin_lottery_core_mobile_index_8'].'';
		}
		else 
		{
			$is_reward = 1;
		...
		{
			load()->func('logging');
			logging_run(''.$this->lang['lang_plugin_lottery_core_mobile_index_9'].'');
		}
	...
	public function reward() 
	{
		...
		if (empty($lottery)) 
		{
			$info = array('status' => 0, 'info' => ''.$this->lang['lang_plugin_lottery_core_mobile_index_10'].'');
		...
		if ($res !== false) 
		{
			$info = array('status' => 1, 'info' => ''.$this->lang['lang_plugin_lottery_core_mobile_index_11'].'' . $temreward[$reward_id]['title']);
			echo json_encode($info);
			exit();
			return;
		}
		$info = array('status' => 0, 'info' => ''.$this->lang['lang_plugin_lottery_core_mobile_index_12'].'');
		echo json_encode($info);
		exit();
	}
	...
	public function checkSubmit($key, $time = 2, $message = ''.$this->lang['lang_plugin_lottery_core_mobile_index_13'].'!') 
	{
		global $_W;
