...
class Recharge_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_app_core_mobile_member_recharge_0'].'!', '', 'error');
		}
	...
	public function submit() 
	{
		...
		if ($money <= 0) 
		{
			show_json(0, ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_1'].'0!');
		}
		if (($money < $minimumcharge) && (0 < $minimumcharge)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_2'].'' . $minimumcharge . ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_3'].'!');
		}
		if (empty($money)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_4'].'!');
		}
		pdo_delete('ewei_shop_member_log', array('openid' => $_W['openid'], 'status' => 0, 'type' => 0, 'uniacid' => $_W['uniacid']));
		$logno = m('common')->createNO('member_log', 'logno', 'RC');
		$log = array('uniacid' => $_W['uniacid'], 'logno' => $logno, 'title' => $set['shop']['name'] . ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_5'].'', 'openid' => $_W['openid'], 'money' => $money, 'type' => 0, 'createtime' => time(), 'status' => 0, 'couponid' => intval($_GPC['couponid']));
		...
		if ($type == 'wechat') 
		{
			if (!(is_weixin())) 
			{
				show_json(0, ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_6'].'!');
			}
			if (empty($set['pay']['weixin']) && empty($set['pay']['weixin_jie'])) 
			{
				show_json(0, ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_7'].'!');
			}
			$wechat = array('success' => false);
			$jie = intval($_GPC['jie']);
			$params = array();
			...
			if (!($wechat['success'])) 
			{
				show_json(0, ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_8'].'!');
			}
		...
		show_json(0, ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_9'].'');
	}
		...
		if (!(empty($log))) 
		{
			...
			if (!(is_error($payquery)) || !(is_error($payqueryborrow))) 
			{
				if (empty($log['status'])) 
				{
					...
					m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, $_W['shopset']['shop']['name'] . ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_10'].':wechatcomplete:credit2:' . $log['money']));
	...
	public function alipay_complete() 
	{
		...
		if (empty($logno)) 
		{
			exit('[ERR1]'.$this->lang['lang_plugin_app_core_mobile_member_recharge_11'].'!');
		}
		if (!(m('finance')->isAlipayNotify($_GET))) 
		{
			exit('[ERR2]'.$this->lang['lang_plugin_app_core_mobile_member_recharge_12'].'!');
		}
		...
		if (!(empty($log)) && empty($log['status'])) 
		{
			...
			m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, $_W['shopset']['shop']['name'] . ''.$this->lang['lang_plugin_app_core_mobile_member_recharge_13'].':alipayreturn:credit2:' . $log['money']));
			m('member')->setRechargeCredit($log['openid'], $log['money']);
			com_run('sale::setRechargeActivity', $log);
			com_run('coupon::useRechargeCoupon', $log);
