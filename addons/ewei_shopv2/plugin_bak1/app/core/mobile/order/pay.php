...
class Pay_EweiShopV2Page extends AppMobilePage 
{
	public function main() 
	{
		...
		$param_title = $set['shop']['name'] . ''.$this->lang['lang_plugin_app_core_mobile_order_pay_0'].'';
	...
	public function complete() 
	{
		...
		foreach ($order_goods as $data ) 
		{
			...
			{
				app_error(AppError::$OrderPayFail, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_pay_1'].'!');
			}
			$unit = ((empty($data['unit']) ? ''.$this->lang['lang_plugin_app_core_mobile_order_pay_2'].'' : $data['unit']));
			if (0 < $data['minbuy']) 
			{
				if ($data['buycount'] < $data['minbuy']) 
				{
					app_error(AppError::$OrderCreateMinBuyLimit, $data['title'] . '<br/> ' . $data['min'] . $unit . ''.$this->lang['lang_plugin_app_core_mobile_order_pay_3'].'!');
				}
			}
			if (0 < $data['maxbuy']) 
			{
				...
				{
					app_error(AppError::$OrderCreateOneBuyLimit, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_pay_4'].' ' . $data['maxbuy'] . $unit . '!');
				}
			}
			if (0 < $data['usermaxbuy']) 
			{
				...
				{
					app_error(AppError::$OrderCreateMaxBuyLimit, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_pay_5'].' ' . $data['usermaxbuy'] . $unit);
				}
			}
			if ($data['istime'] == 1) 
			{
				...
				{
					app_error(AppError::$OrderCreateTimeNotStart, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_pay_6'].'!');
				}
				if ($data['timeend'] < time()) 
				{
					app_error(AppError::$OrderCreateTimeEnd, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_pay_7'].'!');
				}
			}
			if ($data['buylevels'] != '') 
			{
				...
				{
					app_error(AppError::$OrderCreateMemberLevelLimit, ''.$this->lang['lang_plugin_app_core_mobile_order_pay_8'].'<br/>' . $data['title'] . '!');
				}
			}
			if ($data['buygroups'] != '') 
			{
				...
				{
					app_error(AppError::$OrderCreateMemberGroupLimit, ''.$this->lang['lang_plugin_app_core_mobile_order_pay_9'].'<br/>' . $data['title'] . '!');
				}
			}
			if ($data['totalcnf'] == 1) 
			{
				if (!(empty($data['optionid']))) 
				{
					...
					if (!(empty($option))) 
					{
						if ($option['stock'] != -1) 
						{
							...
							{
								app_error(AppError::$OrderCreateStockError, $data['title'] . '<br/>' . $option['title'] . ' '.$this->lang['lang_plugin_app_core_mobile_order_pay_10'].'!');
							}
						}
					}
				}
				else if ($data['stock'] != -1) 
				{
					if (empty($data['stock'])) 
					{
						app_error(AppError::$OrderCreateStockError, $data['title'] . '<br/>' . $option['title'] . ' '.$this->lang['lang_plugin_app_core_mobile_order_pay_11'].'!');
					}
				}
			}
		}
		if ($type == 'cash') 
		{
			...
			{
				app_error(AppError::$OrderPayFail, ''.$this->lang['lang_plugin_app_core_mobile_order_pay_12'].'');
			}
		...
		if ($type == 'credit') 
		{
			...
			{
				app_error(AppError::$OrderPayFail, ''.$this->lang['lang_plugin_app_core_mobile_order_pay_13'].'');
			}
			if ($ps['fee'] < 0) 
			{
				app_error(AppError::$OrderPayFail, ''.$this->lang['lang_plugin_app_core_mobile_order_pay_14'].'');
			}
			$credits = m('member')->getCredit($openid, 'credit2');
			if ($credits < $ps['fee']) 
			{
				app_error(AppError::$OrderPayFail, ''.$this->lang['lang_plugin_app_core_mobile_order_pay_15'].','.$this->lang['lang_plugin_app_core_mobile_order_pay_16'].'');
			}
			$fee = floatval($ps['fee']);
			$shopset = m('common')->getSysset('shop');
			$result = m('member')->setCredit($openid, 'credit2', -$fee, array($_W['member']['uid'], $shopset['name'] . 'APP '.$this->lang['lang_plugin_app_core_mobile_order_pay_17'].'' . $fee));
			if (is_error($result)) 
			{
		...
		else if ($type == 'wechat') 
		{
			...
			{
				app_error(AppError::$OrderPayFail, ''.$this->lang['lang_plugin_app_core_mobile_order_pay_18'].'');
			}
	...
	protected function success($orderid) 
	{
		...
		if (!(empty($address))) 
		{
			$text = ''.$this->lang['lang_plugin_app_core_mobile_order_pay_19'].'';
		}
		if (!(empty($order['dispatchtype'])) && empty($order['isverify'])) 
		{
			$text = ''.$this->lang['lang_plugin_app_core_mobile_order_pay_20'].'';
		}
		if (!(empty($order['isverify']))) 
		{
			$text = ''.$this->lang['lang_plugin_app_core_mobile_order_pay_21'].'';
		}
		if (!(empty($order['virtual']))) 
		{
			$text = ''.$this->lang['lang_plugin_app_core_mobile_order_pay_22'].'';
		}
		if (!(empty($order['isvirtual'])) && empty($order['virtual'])) 
		{
			if (!(empty($order['isvirtualsend']))) 
			{
				$text = ''.$this->lang['lang_plugin_app_core_mobile_order_pay_23'].'';
			}
			else 
			{
				$text = ''.$this->lang['lang_plugin_app_core_mobile_order_pay_24'].'';
			}
		}
		app_json(array( 'order' => array('id' => $orderid, 'isverify' => $order['isverify'], 'virtual' => $order['virtual'], 'isvirtual' => $order['isvirtual'], 'isvirtualsend' => $order['isvirtualsend'], 'status' => ($order['paytype'] == 3 ? ''.$this->lang['lang_plugin_app_core_mobile_order_pay_25'].'' : ''.$this->lang['lang_plugin_app_core_mobile_order_pay_26'].''), 'text' => $text), 'paytype' => ($order['paytype'] == 3 ? ''.$this->lang['lang_plugin_app_core_mobile_order_pay_27'].'' : ''.$this->lang['lang_plugin_app_core_mobile_order_pay_28'].''), 'carrier' => $carrier, 'address' => $address, 'stores' => $stores, 'store' => $store ));
	}
