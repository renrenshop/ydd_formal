...
class Create_EweiShopV2Page extends AppMobilePage 
{
	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			else 
			{
				...
				{
					$followtip = ((empty($goods['followtip']) ? ''.$this->lang['lang_plugin_app_core_mobile_order_create_0'].'~' : $goods['followtip']));
	...
	public function caculatecoupon($couponid, $goodsarr, $totalprice, $discountprice, $isdiscountprice, $isSubmit = 0, $discountprice_array = array(), $merchisdiscountprice = 0) 
	{
		...
		if (is_array($goodsarr)) 
		{
			...
			if ((0 < $deduct) && ($backtype == 0) && (0 < $coupongoodprice)) 
			{
				...
				$deductprice = $deduct;
				$coupondeduct_text = ''.$this->lang['lang_plugin_app_core_mobile_order_create_1'].'';
			...
			else if ((0 < $discount) && ($backtype == 1)) 
			{
				...
				if (0 < $merchid) 
				{
					$coupondeduct_text = ''.$this->lang['lang_plugin_app_core_mobile_order_create_2'].'(' . $discount . ''.$this->lang['lang_plugin_app_core_mobile_order_create_3'].')';
				}
				else 
				{
					$coupondeduct_text = ''.$this->lang['lang_plugin_app_core_mobile_order_create_4'].'(' . $discount . ''.$this->lang['lang_plugin_app_core_mobile_order_create_5'].')';
				}
			}
		}
	...
	public function submit() 
	{
		...
		foreach ($goods as $g ) 
		{
			...
			{
				app_error(AppError::$GoodsNotFound, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_create_6'].'!');
			}
			$task_goods_data = m('goods')->getTaskGoods($openid, $goodsid, $optionid);
			if (empty($task_goods_data['is_task_goods'])) 
			{
			...
			$unit = ((empty($data['unit']) ? ''.$this->lang['lang_plugin_app_core_mobile_order_create_7'].'' : $data['unit']));
			if (0 < $data['minbuy']) 
			{
				if ($goodstotal < $data['minbuy']) 
				{
					app_error(AppError::$OrderCreateMinBuyLimit, $data['title'] . '<br/> ' . $data['minbuy'] . $unit . ''.$this->lang['lang_plugin_app_core_mobile_order_create_8'].'!');
				}
			}
			if (0 < $data['maxbuy']) 
			{
				...
				{
					app_error(AppError::$OrderCreateOneBuyLimit, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_create_9'].' ' . $data['maxbuy'] . $unit . '!');
				}
			}
			if (0 < $data['usermaxbuy']) 
			{
				...
				{
					app_error(AppError::$OrderCreateMaxBuyLimit, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_create_10'].' ' . $data['usermaxbuy'] . $unit . '!');
				}
			}
			if (!(empty($data['is_task_goods']))) 
			{
				...
				{
					app_error(AppError::$OrderCreateTaskGoodsMaxBuyLimit, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_create_11'].' ' . $data['task_goods']['total'] . $unit . '!');
				}
			}
			if ($data['istime'] == 1) 
			{
				...
				{
					app_error(AppError::$OrderCreateTimeNotStart, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_create_12'].'!');
				}
				if ($data['timeend'] < time()) 
				{
					app_error(AppError::$OrderCreateTimeEnd, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_create_13'].'!');
				}
			}
			...
			if ($data['buylevels'] != '') 
			{
				...
				{
					app_error(AppError::$OrderCreateMemberLevelLimit, ''.$this->lang['lang_plugin_app_core_mobile_order_create_14'].'<br/>' . $data['title'] . '!');
				}
			}
			if ($data['buygroups'] != '') 
			{
				...
				{
					app_error(AppError::$OrderCreateMemberGroupLimit, ''.$this->lang['lang_plugin_app_core_mobile_order_create_15'].'<br/>' . $data['title'] . '!');
				}
			}
			if (!(empty($optionid))) 
			{
				...
				if (!(empty($option))) 
				{
					if ($option['stock'] != -1) 
					{
						...
						{
							app_error(AppError::$OrderCreateStockError, $data['title'] . '<br/>' . $option['title'] . ' '.$this->lang['lang_plugin_app_core_mobile_order_create_16'].'!');
						}
					}
					$data['optionid'] = $optionid;
					$data['optiontitle'] = $option['title'];
			...
			else if ($data['stock'] != -1) 
			{
				if (empty($data['stock'])) 
				{
					app_error(AppError::$OrderCreateStockError, $data['title'] . '<br/> '.$this->lang['lang_plugin_app_core_mobile_order_create_17'].'!');
				}
			}
		...
		{
			app_error(AppError::$OrderCreateTaskGoodsCart, $tgoods['title'] . ''.$this->lang['lang_plugin_app_core_mobile_order_create_18'].','.$this->lang['lang_plugin_app_core_mobile_order_create_19'].'!');
		}
		if (empty($allgoods)) 
		{
			app_error(AppError::$OrderCreateNoGoods);
		...
		if (!($isvirtual) && !($isverify) && ($dispatchtype == 0)) 
		{
			...
			if (!(empty($nodispatch_array['isnodispatch']))) 
			{
				app_error(AppError::$OrderCreateNoDispatch, $nodispatch_array['nodispatch'] . ''.$this->lang['lang_plugin_app_core_mobile_order_create_20'].'!');
			}
		}
		if (empty($goods[0]['bargain_id']) || !(p('bargain'))) 
		{
		...
		if (0 < $deductcredit) 
		{
			m('member')->setCredit($openid, 'credit1', -$deductcredit, array('0', $_W['shopset']['shop']['name'] . ''.$this->lang['lang_plugin_app_core_mobile_order_create_21'].' '.$this->lang['lang_plugin_app_core_mobile_order_create_22'].': ' . $deductcredit . ' '.$this->lang['lang_plugin_app_core_mobile_order_create_23'].': ' . $deductmoney . ' '.$this->lang['lang_plugin_app_core_mobile_order_create_24'].': ' . $ordersn));
		}
		if (0 < $buyagainprice) 
		{
