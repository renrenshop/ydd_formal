define(['core'], function(core) {
	...
	modal.init = function(jie, cashierid, operatorid, id) {
		...
		$(document).off('click', '#btn-wechat').on('click', '#btn-wechat', function() {
			...
			if (isNaN(money) || money <= 0) {
				FoxUI.toast.show(lang_js_cashier_static_js_mobile_0+'0!');
				return
			}
			setTimeout(function() {
				FoxUI.confirm(lang_js_cashier_static_js_mobile_1+' ' + money + ' '+lang_js_cashier_static_js_mobile_2+'?', lang_js_cashier_static_js_mobile_3, function() {
							...
							function onBridgeReady() {
								WeixinJSBridge.invoke('getBrandWCPayRequest', {
									...
									if (res.err_msg == 'get_brand_wcpay_request:ok') {
										...
										$('.btn-pay').removeAttr('submit');
										FoxUI.toast.show(lang_js_cashier_static_js_mobile_4)
									} else {
		...
		$(document).off('click', '#btn-alipay').on('click', '#btn-alipay', function() {
			...
			if (isNaN(money) || money <= 0) {
				FoxUI.toast.show(lang_js_cashier_static_js_mobile_5+'0!');
				return
			}
			FoxUI.loader.show('mini');
			setTimeout(function() {
				FoxUI.confirm(lang_js_cashier_static_js_mobile_6+' ' + money + ' '+lang_js_cashier_static_js_mobile_7+'?', lang_js_cashier_static_js_mobile_8, function() {
					core.json('cashier/pay/pay', {
						cashierid: modal.cashierid,
						operatorid: modal.operatorid,
						couponid: modal.couponid,
	...
	modal.bindCoupon = function(money) {
		...
		coupondiv.unbind('click').click(function() {
			core.json('cashier/pay/getcoupon', {
				money: money,
				cashierid: modal.cashierid
			}, function(rjson) {
				if (rjson.result.coupons.length > 0) {
					...
					require(['biz/sale/coupon/picker'], function(picker) {
						picker.show({
							...
							onCancel: function() {
								...
								coupondiv.find('.fui-cell-label').html(lang_js_cashier_static_js_mobile_9);
								coupondiv.find('.fui-cell-info').html('')
							},
							onSelected: function(data) {
								modal.couponid = data.id || data.couponid;
								modal.couponmerchid = data.merchid;
								coupondiv.find('.fui-cell-label').html(lang_js_cashier_static_js_mobile_10);
					...
					FoxUI.toast.show(lang_js_cashier_static_js_mobile_11+'!');
					modal.hideCoupon()
				}
			}, false, true)
		})
