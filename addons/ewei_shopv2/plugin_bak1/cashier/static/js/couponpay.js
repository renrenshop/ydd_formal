define(['core', 'tpl'], function(core, tpl) {
	...
	modal.init = function(jie, cashierid, operatorid, type) {
		...
		$("#btn-wechat").unbind('click').click(function() {
			...
			if (isNaN(money) || money <= 0) {
				FoxUI.toast.show(lang_js_cashier_static_js_couponpay_0+'0!');
				return
			}
					...
					function onBridgeReady() {
						WeixinJSBridge.invoke('getBrandWCPayRequest', {
							...
							if (res.err_msg == 'get_brand_wcpay_request:ok') {
								...
								$('.btn-pay').removeAttr('submit');
								FoxUI.toast.show(lang_js_cashier_static_js_couponpay_1)
							} else {
								core.json('cashier/couponpay/pay', {
									cashierid: modal.cashierid,
									operatorid: modal.operatorid,
		...
		$("#btn-alipay").unbind('click').click(function() {
			...
			if (isNaN(money) || money <= 0) {
				FoxUI.toast.show(lang_js_cashier_static_js_couponpay_2+'0!');
				return
			}
			core.json('cashier/couponpay/pay', {
				cashierid: modal.cashierid,
	...
	modal.discount = function($money) {
		...
		var $no_money = $('#no_money');
		$.getJSON(core.getUrl("cashier/couponpay/get_discount"), {
			cashierid: modal.cashierid,
			type: modal.type,
			money: $money.val(),
			no_money: $no_money.val()
		}, function(json) {
