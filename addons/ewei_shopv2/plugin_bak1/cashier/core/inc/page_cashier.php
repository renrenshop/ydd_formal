...
class CashierWebPage extends PluginWebPage 
{
	...
	public function __construct($_com = '', $_init = false) 
	{
		...
		if (!(empty($_com))) 
		{
			...
			{
				$this->message(''.$this->lang['lang_plugin_cashier_core_inc_page_cashier_0'].'');
			}
		}
		else 
		{
		...
		if (empty($this->set['isopen'])) 
		{
			...
			{
				$this->message(''.$this->lang['lang_plugin_cashier_core_inc_page_cashier_1'].','.$this->lang['lang_plugin_cashier_core_inc_page_cashier_2'].'!', cashierUrl('quit'));
			}
		}
		if ($_W['cashieruser']['lifetimeend'] < time()) 
		{
			if (($_W['routes'] != 'login') && ($_W['routes'] != 'quit')) 
			{
				$this->message(''.$this->lang['lang_plugin_cashier_core_inc_page_cashier_3'].'!', cashierUrl('quit'));
			}
		}
		if (!($this->model->is_perm($_W['routes'])) && ($_W['routes'] != 'login') && ($_W['routes'] != 'quit') && ($_W['routes'] != 'qr')) 
		{
			$this->message(''.$this->lang['lang_plugin_cashier_core_inc_page_cashier_4'].'!');
		}
	}
