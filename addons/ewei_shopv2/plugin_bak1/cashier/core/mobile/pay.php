...
class Pay_EweiShopV2Page extends CashierMobilePage 
{
	...
	public function pay($params = array(), $mine = array()) 
	{
		...
		if ($money <= 0) 
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_0'].'!');
		}
		$usecoupon = 0;
		...
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_1'].','.$this->lang['lang_plugin_cashier_core_mobile_pay_2'].'!');
		}
	...
	protected function paytype0($jie) 
	{
		...
		if (!($wechat['success'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_3'].'!');
		}
	...
	protected function paytype1() 
	{
		...
		show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_4'].'!');
	}
	protected function paytype101($jie) 
	{
		...
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_5'].'!');
		}
		if (empty($set['pay']['weixin']) && empty($set['pay']['weixin_jie'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_6'].'!');
		}
		...
		if (!($wechat['success'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_7'].'!');
		}
		show_json(1, array('wechat' => $wechat, 'logid' => $this->log['id'], 'log' => $this->log));
	}
	protected function paytype102() 
	{
		...
		show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_8'].'!');
	}
	...
	public function orderquery() 
	{
		...
		show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_pay_9'].'!');
	}
	public function success() 
	{
		...
		if ($log && ($log['status'] == 1)) 
		{
			$item = array('title' => $log['title'], 'goodstitle' => $log['title'] . ''.$this->lang['lang_plugin_cashier_core_mobile_pay_10'].'', 'money' => $log['money'] + $log['deduction'], 'paytype' => $log['paytype'], 'time' => date('Y-m-d H:i:s', $log['paytime']), 'out_trade_no' => $log['logno'], 'randommoney' => (double) $log['randommoney'], 'enough' => (double) $log['enough'], 'deduction' => (double) $log['deduction'], 'discountmoney' => (double) $log['discountmoney'], 'orderprice' => (double) $log['orderprice'], 'goodsprice' => (double) $log['goodsprice'], 'couponpay' => (double) $log['usecouponprice'], 'present_credit1' => (int) $log['present_credit1']);
			if (empty($log['paytype']) || ($log['paytype'] == '101')) 
			{
				$item['paytype'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_11'].'';
			}
			else 
			{
				if (($log['paytype'] == '1') || ($log['paytype'] == '102')) 
				{
					$item['paytype'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_12'].'';
				}
				else if ($log['paytype'] == '2') 
				{
					$item['paytype'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_13'].'';
				}
				else if ($log['paytype'] == '3') 
				{
					$item['paytype'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_14'].'';
				}
			}
			if (!(empty($log['coupon']))) 
			{
				...
				if (!(empty($coupon))) 
				{
					...
					$coupon['timestr'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_15'].'';
					...
					{
						$coupon['backstr'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_16'].'';
						$coupon['css'] = 'deduct';
					...
					{
						$coupon['backstr'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_17'].'';
						$coupon['css'] = 'discount';
					...
					else if ($coupon['backtype'] == 2) 
					{
						...
						{
							$coupon['backstr'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_18'].'';
							$coupon['css'] = 'redpack';
						...
						{
							$coupon['backstr'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_19'].'';
							$coupon['css'] = 'money';
							$coupon['backpre'] = true;
						}
						else if (!(empty($coupon['backcredit']))) 
						{
							$coupon['backstr'] = ''.$this->lang['lang_plugin_cashier_core_mobile_pay_20'].'';
							$coupon['css'] = 'credit';
