...
class Couponpay_EweiShopV2Page extends CashierMobilePage 
{
	...
	public function main() 
	{
		...
		$id = false;
		$goodstitle = $_W['cashieruser']['title'] . ''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_0'].'';
		$couponpay = $this->model->getUserSet('couponpay', $_W['cashierid']);
		if (empty($couponpay['goodsids'])) 
		{
			$this->message(''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_1'].'!', '', 'error');
		}
	...
	public function pay() 
	{
		...
		if ($money <= 0) 
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_2'].'!');
		}
		$couponpay = $this->model->getUserSet('couponpay', $_W['cashierid']);
		if (!(empty($type))) 
		{
		...
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_3'].','.$this->lang['lang_plugin_cashier_core_mobile_couponpay_4'].'!');
		}
		if ($paytype == '0') 
		{
			$this->paytype0($jie);
	...
	protected function paytype0($jie) 
	{
		...
		if (!($wechat['success'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_5'].'!');
		}
	...
	protected function paytype1() 
	{
		...
		show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_6'].'!');
	}
	protected function paytype101($jie) 
	{
		...
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_7'].'!');
		}
		if (empty($set['pay']['weixin']) && empty($set['pay']['weixin_jie'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_8'].'!');
		}
		...
		if (!($wechat['success'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_cashier_core_mobile_couponpay_9'].'!');
		}
		show_json(1, array('wechat' => $wechat, 'logid' => $this->log['id']));
	}
