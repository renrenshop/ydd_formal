...
class Register_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_merch_core_mobile_register_0'].'', '', 'error');
		}
		...
		{
			$this->message(''.$this->lang['lang_plugin_merch_core_mobile_register_1'].'!', '', 'error');
		}
		$apply_set = array();
		$apply_set['open_protocol'] = $set['open_protocol'];
		if (empty($set['applytitle'])) 
		{
			$apply_set['applytitle'] = ''.$this->lang['lang_plugin_merch_core_mobile_register_2'].'';
		}
		else 
		{
		...
		if ($_W['ispost']) 
		{
			if (empty($set['apply_openmobile'])) 
			{
				show_json(0, ''.$this->lang['lang_plugin_merch_core_mobile_register_3'].'!');
			}
			if (!(empty($user)) && (1 <= $user['status'])) 
			{
				show_json(0, ''.$this->lang['lang_plugin_merch_core_mobile_register_4'].'!');
			}
			$uname = trim($_GPC['uname']);
			$upass = $_GPC['upass'];
			if (empty($uname)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_merch_core_mobile_register_5'].'!');
			}
			if (empty($upass)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_merch_core_mobile_register_6'].'!');
			}
			$where1 = ' uname=:uname';
			...
			{
				show_json(0, ''.$this->lang['lang_plugin_merch_core_mobile_register_7'].' ' . $uname . ' '.$this->lang['lang_plugin_merch_core_mobile_register_8'].','.$this->lang['lang_plugin_merch_core_mobile_register_9'].'!');
			}
