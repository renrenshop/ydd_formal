...
class MerchWebPage extends PluginWebPage 
{
	...
	public function __construct($_com = '', $_init = false) 
	{
		...
		if (!(empty($_com))) 
		{
			...
			{
				$this->message(''.$this->lang['lang_plugin_merch_core_inc_page_merch_0'].'');
			}
		}
		else 
		{
		...
		if (($_W['merchisfounder'] != '1') && ($_W['routes'] != 'shop')) 
		{
			...
			if ($is_xxx) 
			{
				if (!($perm)) 
				{
					foreach ($is_xxx as $item ) 
					{
						...
						{
							$this->message(''.$this->lang['lang_plugin_merch_core_inc_page_merch_1'].'');
						}
					}
					return;
				}
			...
			else 
			{
				...
				else 
				{
					...
					{
						$this->message(''.$this->lang['lang_plugin_merch_core_inc_page_merch_2'].'');
					}
					else if (!($perm) && in_array($_W['routes'], $perm_type_value)) 
					{
						$this->message(''.$this->lang['lang_plugin_merch_core_inc_page_merch_3'].'');
					}
				}
				if (isset($perm_view) && !($perm_view)) 
				{
					$this->message(''.$this->lang['lang_plugin_merch_core_inc_page_merch_4'].'');
				}
			}
		}
	}
