...
class Qrcode_EweiShopV2Page extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		if (!empty($set['closed_qrcode']) && !intval($_GPC['goodsid'])) {
			$this->message(''.$this->lang['lang_plugin_commission_core_mobile_qrcode_0'].'!', mobileUrl('commission'), 'info');
		}


		if (empty($set['become_reg'])) {
			if (empty($member['realname'])) {
				$this->message(''.$this->lang['lang_plugin_commission_core_mobile_qrcode_1'].'!', mobileUrl('member/info', array('returnurl' => $returnurl)), 'info');
			}
