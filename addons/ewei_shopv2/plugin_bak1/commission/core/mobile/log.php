...
class Log_EweiShopV2Page extends CommissionMobileLoginPage
{
	...
	public function get_list()
	{
		...
		foreach ($list as &$row ) {
			if ($row['status'] == 1) {
				$row['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_0'].'';
				$row['dealtime'] = date('Y-m-d H:i', $row['applytime']);
			}
			 else if ($row['status'] == 2) {
				$row['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_1'].'';
				$row['dealtime'] = date('Y-m-d H:i', $row['checktime']);
			}
			 else if ($row['status'] == 3) {
				$row['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_2'].'';
				$row['dealtime'] = date('Y-m-d H:i', $row['paytime']);
			}
			 else if ($row['status'] == -1) {
				$row['dealtime'] = date('Y-m-d H:i', $row['invalidtime']);
				$row['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_3'].'';
			}
			 else if ($row['status'] == -2) {
				$row['dealtime'] = date('Y-m-d H:i', $row['refusetime']);
				$row['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_4'].'';
			}
	...
	public function detail()
	{
		...
		if (empty($apply)) {
			$this->message(''.$this->lang['lang_plugin_commission_core_mobile_log_5'].'!', '', 'error');
		}


		$orderids = iunserializer($apply['orderids']);
		if (!is_array($orderids) || (count($orderids) <= 0)) {
			$this->message(''.$this->lang['lang_plugin_commission_core_mobile_log_6'].'!', '', 'error');
		}
	...
	public function orders()
	{
		...
		if (empty($apply)) {
			show_json(0, array('message' => ''.$this->lang['lang_plugin_commission_core_mobile_log_7'].'!'));
		}


		$orderids = iunserializer($apply['orderids']);
		if (!is_array($orderids) || (count($orderids) <= 0)) {
			show_json(0, array('message' => ''.$this->lang['lang_plugin_commission_core_mobile_log_8'].'!'));
		}
		...
		foreach ($list as &$row ) {
			...
			foreach ($goods as &$g ) {
				...
				if ($status == 1) {
					$g['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_9'].'';
					$g['dealtime'] = date('Y-m-d H:i', $row['applytime' . $row['level']]);
				}
				 else if ($status == 2) {
					$g['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_10'].'';
					$g['dealtime'] = date('Y-m-d H:i', $row['checktime' . $row['level']]);
				}
				 else if ($status == 3) {
					$g['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_11'].'';
					$g['dealtime'] = date('Y-m-d H:i', $row['checktime' . $row['level']]);
				}
				 else if ($status == -1) {
					$g['dealtime'] = date('Y-m-d H:i', $row['invalidtime' . $row['level']]);
					$g['statusstr'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_12'].'';
				}
				...
				if ($row['level'] == 1) {
					$g['level'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_13'].'';
				}
				 else if ($row['level'] == 2) {
					$g['level'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_14'].'';
				}
				 else if ($row['level'] == 3) {
					$g['level'] = ''.$this->lang['lang_plugin_commission_core_mobile_log_15'].'';
				}
