	...
	public function main() 
	{
		...
		if (empty($become_reg)) 
		{
			if (empty($member['realname'])) 
			{
				...
				$this->message(''.$this->lang['lang_plugin_commission_core_mobile_apply_0'].'!', mobileUrl('member/info', array('returnurl' => $returnurl)), 'info');
			}
		}
		$time = time();
		...
		{
			$type_array[0]['title'] = $this->set['texts']['withdraw'] . ''.$this->lang['lang_plugin_commission_core_mobile_apply_1'].'' . $_W['shopset']['trade']['moneytext'];
		}
		if (($this->set['cashweixin'] == 1) && !(is_h5app())) 
		{
			$type_array[1]['title'] = $this->set['texts']['withdraw'] . ''.$this->lang['lang_plugin_commission_core_mobile_apply_2'].'';
		}
		if ($this->set['cashother'] == 1) 
		{
			if ($this->set['cashalipay'] == 1) 
			{
				$type_array[2]['title'] = $this->set['texts']['withdraw'] . ''.$this->lang['lang_plugin_commission_core_mobile_apply_3'].'';
				if (!(empty($last_data))) 
				{
					if ($last_data['type'] != 2) 
					{
			...
			{
				$type_array[3]['title'] = $this->set['texts']['withdraw'] . ''.$this->lang['lang_plugin_commission_core_mobile_apply_4'].'';
				if (!(empty($last_data))) 
				{
					if ($last_data['type'] != 3) 
					{
		...
		if ($_W['ispost']) 
		{
			...
			{
				show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_5'].'!');
			}
			unset($_SESSION['commission_apply_token']);
			if (($commission_ok <= 0) || empty($orderids)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_6'].','.$this->lang['lang_plugin_commission_core_mobile_apply_7'].'!');
			}
			$type = intval($_GPC['type']);
			if (!(array_key_exists($type, $type_array))) 
			{
				show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_8'].'!');
			}
			$apply = array();
			if ($type == 2) 
			{
				...
				if (empty($realname)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_9'].'!');
				}
				if (empty($alipay)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_10'].'!');
				}
				if (empty($alipay1)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_11'].'!');
				}
				if ($alipay != $alipay1) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_12'].'!');
				}
			...
			else if ($type == 3) 
			{
				...
				if (empty($realname)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_13'].'!');
				}
				if (empty($bankname)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_14'].'!');
				}
				if (empty($bankcard)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_15'].'!');
				}
				if (empty($bankcard1)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_16'].'!');
				}
				if ($bankcard != $bankcard1) 
				{
					show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_apply_17'].'!');
				}
				$apply['realname'] = $realname;
			...
			$apply_type = array(''.$this->lang['lang_plugin_commission_core_mobile_apply_18'].'', ''.$this->lang['lang_plugin_commission_core_mobile_apply_19'].'', ''.$this->lang['lang_plugin_commission_core_mobile_apply_20'].'', ''.$this->lang['lang_plugin_commission_core_mobile_apply_21'].'');
			$mcommission = $commission_ok;
			if (!(empty($deductionmoney))) 
			{
				$mcommission .= ','.$this->lang['lang_plugin_commission_core_mobile_apply_22'].':' . $realmoney . ','.$this->lang['lang_plugin_commission_core_mobile_apply_23'].':' . $deductionmoney;
			}
			$this->model->sendMessage($openid, array('commission' => $mcommission, 'type' => $apply_type[$apply['type']]), TM_COMMISSION_APPLY);
			show_json(1, ''.$this->lang['lang_plugin_commission_core_mobile_apply_24'].','.$this->lang['lang_plugin_commission_core_mobile_apply_25'].'!');
		}
		$token = md5(microtime());
