...
class Select_EweiShopV2Page extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		if ($member['agentselectgoods'] == 1) {
			$err = ''.$this->lang['lang_plugin_commission_core_mobile_myshop_select_0'].'!';
		...
		if (empty($this->set['select_goods'])) {
			if ($member['agentselectgoods'] != 2) {
				$err = ''.$this->lang['lang_plugin_commission_core_mobile_myshop_select_1'].'!';
		...
		if ($_W['ispost']) {
			...
			if (!empty($shopdata['selectgoods']) && !is_array($_GPC['goodsids'])) {
				show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_myshop_select_2'].'!');
			}
