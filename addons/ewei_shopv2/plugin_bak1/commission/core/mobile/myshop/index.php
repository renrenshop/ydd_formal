...
class Index_EweiShopV2Page extends PluginMobilePage 
{
	...
	public function select() 
	{
		...
		{
			$err = ''.$this->lang['lang_plugin_commission_core_mobile_myshop_index_0'].'!';
			if ($_W['ispost']) 
			{
				show_json(-1, $err);
			}
		...
		if (empty($this->set['select_goods'])) 
		{
			...
			{
				$err = ''.$this->lang['lang_plugin_commission_core_mobile_myshop_index_1'].'!';
				if ($_W['ispost']) 
				{
					show_json(-1, $err);
				}
		...
		if ($_W['ispost']) 
		{
			...
			{
				show_json(0, ''.$this->lang['lang_plugin_commission_core_mobile_myshop_index_2'].'!');
			}
			if (empty($shop['id'])) 
			{
				pdo_insert('ewei_shop_commission_shop', $shopdata);
