	...
	modal.init = function(mid, params) {
		...
		$('.btn-submit').click(function() {
			...
			if ($(".diyform-container").length > 0) {
				...
				if ($('#realname').isEmpty()) {
					FoxUI.toast.show(lang_js_commission_static_js_register_0+'!');
					return
				}
				if (!$('#mobile').isMobile()) {
					FoxUI.toast.show(lang_js_commission_static_js_register_1+'!');
					return
				}
				data = {
					'agentid': mid,
			...
			if (modal.params.open_protocol == 1) {
				if (!$('#agree').prop('checked')) {
					FoxUI.toast.show(lang_js_commission_static_js_register_2+'【' + modal.params.applytitle + '】!');
					return
				}
			}
			btn.attr('stop', 1).html(lang_js_commission_static_js_register_3+'...');
			core.json('commission/register', data, function(pjson) {
				...
				if (result.check == '1') {
					FoxUI.message.show({
						...
						content: ""+lang_js_commission_static_js_register_4+"!",
						buttons: [{
							text: lang_js_commission_static_js_register_5,
					...
					FoxUI.message.show({
						...
						content: ""+lang_js_commission_static_js_register_6+"，"+lang_js_commission_static_js_register_7+"!",
						buttons: [{
							text: lang_js_commission_static_js_register_8,
