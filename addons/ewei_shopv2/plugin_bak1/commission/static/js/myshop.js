define(['core', 'tpl', 'biz/goods/picker'], function(core, tpl, picker) {
	...
	modal.initSet = function() {
		...
		$('.btn-submit').click(function() {
			...
			btn.attr('stop', 1).html(lang_js_commission_static_js_myshop_0+'...');
	...
	modal.initSelect = function() {
		...
		$('.btn-submit').click(function() {
			...
			if (goodsids.length <= 0 && $('#openselect').prop('checked')) {
				FoxUI.toast.show(lang_js_commission_static_js_myshop_1+'!');
				return
			}
			var data = {
			...
			btn.attr('stop', 1).html(lang_js_commission_static_js_myshop_2+'...');
