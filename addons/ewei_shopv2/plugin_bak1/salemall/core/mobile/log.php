...
class Log_EweiShopV2Page extends MobilePage 
{
	...
	public function detail() 
	{
		...
		if (empty($log)) 
		{
			show_json(-1, ''.$this->lang['lang_plugin_salemall_core_mobile_log_0'].'!');
		}
		$goods = $this->model->getGoods($log['goodsid'], $member);
		if (empty($goods['id'])) 
		{
			show_json(-1, ''.$this->lang['lang_plugin_salemall_core_mobile_log_1'].'!');
		}
		$address = false;
	...
	public function paydispatch() 
	{
		...
		if (empty($log)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_2'].'!');
		}
		$goods = $this->model->getGoods($log['goodsid'], $member);
		if (empty($goods['id'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_3'].'!');
		}
		if (!(empty($goods['isendtime']))) 
		{
			if ($goods['endtime'] < time()) 
			{
				show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_4'].'!');
			}
		}
		if ($goods['dispatch'] <= 0) 
		{
		...
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_5'].'!');
		}
		...
		if (!(is_weixin())) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_6'].'!');
		}
		if (empty($set['pay']['weixin']) && empty($set['pay']['weixin_jie'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_7'].'!');
		}
		...
		$params['title'] = $set['shop']['name'] . ((empty($goods['type']) ? ''.$this->lang['lang_plugin_salemall_core_mobile_log_8'].'' : ''.$this->lang['lang_plugin_salemall_core_mobile_log_9'].'')) . ' '.$this->lang['lang_plugin_salemall_core_mobile_log_10'].':' . $dispatchno;
		...
		if (!($wechat['success'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_11'].'!');
		}
	...
	public function payresult($a = array()) 
	{
		...
		if (empty($log)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_12'].'!');
		}
		if (empty($log['dispatchstatus'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_13'].'!');
		}
		$goods = $this->model->getGoods($log['goodsid'], $member);
		if (empty($goods['id'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_14'].'!');
		}
	...
	public function setstore() 
	{
		...
		if (empty($storeid)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_15'].'!');
		}
		$log = pdo_fetch('select * from ' . tablename('ewei_shop_creditshop_log') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
		if (empty($log)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_16'].'!');
		}
		$goods = $this->model->getGoods($log['goodsid'], $member);
		if (empty($goods['id'])) 
		{
			show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_log_17'].'!');
		}
		$upgrade = array();
		$upgradem = array();
		if (empty($log['storeid'])) 
