...
class Detail_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_salemall_core_mobile_detail_0'].'!', mobileUrl('creditshop'), 'error');
		}
		if ($goods['type'] == 0) 
		{
			$stores = array();
	...
	public function pay($a = array(), $b = array()) 
	{
		...
		if ($needpay) 
		{
			...
			if ($useweixin) 
			{
				...
				if (!(is_weixin())) 
				{
					show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_detail_1'].'!');
				}
				if (empty($set['pay']['weixin']) && empty($set['pay']['weixin_jie'])) 
				{
					show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_detail_2'].'!');
				}
				...
				$params['title'] = $set['shop']['name'] . ((empty($goods['type']) ? ' ' . $_W['shopset']['trade']['credittext'] . ''.$this->lang['lang_plugin_salemall_core_mobile_detail_3'].'' : '' . $_W['shopset']['trade']['credittext'] . ''.$this->lang['lang_plugin_salemall_core_mobile_detail_4'].'')) . ' '.$this->lang['lang_plugin_salemall_core_mobile_detail_5'].':' . $log['logno'];
				...
				if (!($wechat['success'])) 
				{
					show_json(0, ''.$this->lang['lang_plugin_salemall_core_mobile_detail_6'].'!');
				}
	...
	public function lottery() 
	{
		...
		if (empty($log)) 
		{
			show_json(-1, ''.$this->lang['lang_plugin_salemall_core_mobile_detail_7'].'!');
		}
		if (1 <= $log['status']) 
		{
			show_json(-1, ''.$this->lang['lang_plugin_salemall_core_mobile_detail_8'].'!');
		}
		if (empty($goods['canbuy'])) 
		{
		...
		if (empty($log['paystatus'])) 
		{
			...
			{
				show_json(-1, '' . $_W['shopset']['trade']['credittext'] . ''.$this->lang['lang_plugin_salemall_core_mobile_detail_9'].'!');
			}
			if ((0 < $goods['money']) && ($money < $goods['money']) && ($log['paytype'] == 0)) 
			{
				show_json(-1, ''.$this->lang['lang_plugin_salemall_core_mobile_detail_10'].'!');
			}
		}
		if ((0 < $goods['money']) && empty($log['paystatus'])) 
		{
			if ($log['paytype'] == 0) 
			{
				m('member')->setCredit($openid, 'credit2', -$goods['money'], '' . $_W['shopset']['trade']['credittext'] . ''.$this->lang['lang_plugin_salemall_core_mobile_detail_11'].' ' . $goods['money']);
				$update['paystatus'] = 1;
			}
			if ($log['paytype'] == 1) 
			{
				if (empty($log['paystatus'])) 
				{
					show_json(-1, ''.$this->lang['lang_plugin_salemall_core_mobile_detail_12'].'!');
				}
			}
		}
		if ((0 < $goods['credit']) && empty($log['creditpay'])) 
		{
			m('member')->setCredit($openid, 'credit1', -$goods['credit'], '' . $_W['shopset']['trade']['credittext'] . ''.$this->lang['lang_plugin_salemall_core_mobile_detail_13'].'' . $_W['shopset']['trade']['credittext'] . ' ' . $goods['credit']);
			$update['creditpay'] = 1;
		}
		$status = 1;
		if (!(empty($goods['type']))) 
