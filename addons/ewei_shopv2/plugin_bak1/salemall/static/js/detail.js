	...
	modal.init = function(params) {
		...
		$('#btnsub').click(function() {
			...
			if (goods.followneed == '1' && !goods.followed) {
				FoxUI.message.show({
					title: ""+lang_js_salemall_static_js_detail_0+"",
					icon: 'icon icon-information',
					content: goods.followtext,
					buttons: [{
						text: lang_js_salemall_static_js_detail_1,
			...
			if (goods.type == 0) {
				if (goods.isverify == 1) {
					if (modal.realname == '') {
						FoxUI.toast.show(lang_js_salemall_static_js_detail_2+'!');
						return
					}
					if (modal.mobile == '') {
						FoxUI.toast.show(lang_js_salemall_static_js_detail_3+'!');
						return
					}
					if (!modal.storeid) {
						FoxUI.toast.show(lang_js_salemall_static_js_detail_4+'!');
						return
					}
				}
				FoxUI.message.show({
					title: ""+lang_js_salemall_static_js_detail_5+"？",
					icon: 'icon icon-information',
					content: '',
					buttons: [{
						text: lang_js_salemall_static_js_detail_6,
						extraClass: 'btn-danger',
						onclick: function() {
							modal.pay(goods)
						}
					}, {
						text: lang_js_salemall_static_js_detail_7,
...
/*
				...
				if (wechat.weixin) {
					WeixinJSBridge.invoke('getBrandWCPayRequest', {
						...
						if (res.err_msg == 'get_brand_wcpay_request:ok') {
							...
							FoxUI.toast.show(lang_js_salemall_static_js_detail_8)
						} else {
							core.json('creditshop/detail/pay', {
								id: modal.goods.id,
								storeid: modal.storeid,
		...
		if (type == 0) {
			...
			core.json('creditshop/detail/lottery', {
				...
				if (json.status == 2) {
					setTimeout(function() {
						FoxUI.message.show({
							title: ""+lang_js_salemall_static_js_detail_9+"，"+lang_js_salemall_static_js_detail_10+"!",
							icon: 'icon icon-success',
							content: '',
							buttons: [{
								text: lang_js_salemall_static_js_detail_11,
								extraClass: 'btn-danger',
								onclick: function() {
					...
					setTimeout(function() {
						FoxUI.message.show({
							title: ""+lang_js_salemall_static_js_detail_12+"，"+lang_js_salemall_static_js_detail_13+"!",
							icon: 'icon icon-success',
							content: '',
							buttons: [{
								text: lang_js_salemall_static_js_detail_14,
								extraClass: 'btn-danger',
								onclick: function() {
									location.href = core.getUrl('creditshop/log', {
										shine: 1
			...
			FoxUI.message.show({
				...
				content: lang_js_salemall_static_js_detail_15+"，"+lang_js_salemall_static_js_detail_16+'....',
				buttons: []
			});
			setTimeout(function() {
				core.json('creditshop/detail/lottery', {
					...
					if (json.status == 2) {
						FoxUI.message.show({
							title: ""+lang_js_salemall_static_js_detail_17+"，"+lang_js_salemall_static_js_detail_18+"!",
							icon: 'icon icon-success',
							content: '',
							buttons: [{
								text: lang_js_salemall_static_js_detail_19,
								extraClass: 'btn-danger',
								onclick: function() {
									location.href = core.getUrl('creditshop/log', {
										shine: 1
						...
						FoxUI.message.show({
							title: ""+lang_js_salemall_static_js_detail_20+"，"+lang_js_salemall_static_js_detail_21+"!",
							icon: 'icon icon-success',
							content: '',
							buttons: [{
								text: lang_js_salemall_static_js_detail_22,
								extraClass: 'btn-danger',
								onclick: function() {
									location.href = core.getUrl('creditshop/log', {
										shine: 1
						...
						FoxUI.message.show({
							title: ""+lang_js_salemall_static_js_detail_23+"，"+lang_js_salemall_static_js_detail_24+"!",
							icon: 'icon icon-wrong',
							content: '',
							buttons: [{
								text: lang_js_salemall_static_js_detail_25,
								extraClass: 'btn-danger',
								onclick: function() {
									location.reload()
								}
	...
	modal.useAppPay = function(json) {
		FoxUI.message.show({
			title: '',
			icon: 'icon icon-clock',
			content: lang_js_salemall_static_js_detail_26,
			buttons: []
		});
		setTimeout(function() {
