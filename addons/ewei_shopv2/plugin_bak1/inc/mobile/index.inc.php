...
$param['appsecret']=$appsecret;
	...
	$json='{"msg":"'.$this->lang['lang_plugin_inc_mobile_index.inc_0'].'!","status":"0"}'; 
	echo $json;
	exit();
} 
...
if($status){
	...
	if (!empty($result)){
	header('Content-type:text/json'); 
	$json='{"msg":"'.$this->lang['lang_plugin_inc_mobile_index.inc_1'].'","status":"1"}'; 
	echo $json;
	exit();
	}else{
	header('Content-type:text/json'); 
	$json='{"msg":"'.$this->lang['lang_plugin_inc_mobile_index.inc_2'].'","status":"0"}'; 
	echo $json;
	exit();	
	}
	...
	header('Content-type:text/json'); 
	$json='{"msg":"'.$this->lang['lang_plugin_inc_mobile_index.inc_3'].'","status":"0"}'; 
	echo $json;
	exit();	
}
