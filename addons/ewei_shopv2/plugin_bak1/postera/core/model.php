...
class PosteraModel extends PluginModel 
{
	...
	public function getQR($poster, $member) 
	{
		...
		if (empty($qr)) 
		{
			...
			if (empty($result)) 
			{
				return error(-1, ''.$this->lang['lang_plugin_postera_core_model_0'].'');
			}
		...
		else 
		{
			...
			if (($qr['endtime'] < time()) || ($qr['qrtime'] != $poster['timestart'] . $poster['timeend'])) 
			{
				...
				if (empty($result)) 
				{
					return error(-1, ''.$this->lang['lang_plugin_postera_core_model_1'].'');
				}
				$barcode = $result['barcode'];
				$ticket = $result['ticket'];
	...
	public function createPoster($poster, $member, $qr, $upload = true) 
	{
		...
		if (!(empty($qr['goodsid']))) 
		{
			...
			if (empty($goods)) 
			{
				m('message')->sendCustomNotice($member['openid'], ''.$this->lang['lang_plugin_postera_core_model_2'].'');
				exit();
			}
		}
	...
	{
		return array( 'postera' => array('text' => $this->getName(), 'isplugin' => true, 'view' => ''.$this->lang['lang_plugin_postera_core_model_3'].'', 'add' => ''.$this->lang['lang_plugin_postera_core_model_4'].'-log', 'edit' => ''.$this->lang['lang_plugin_postera_core_model_5'].'-log', 'delete' => ''.$this->lang['lang_plugin_postera_core_model_6'].'-log', 'log' => ''.$this->lang['lang_plugin_postera_core_model_7'].'', 'clear' => ''.$this->lang['lang_plugin_postera_core_model_8'].'-log', 'setdefault' => ''.$this->lang['lang_plugin_postera_core_model_9'].'-log') );
	}
}
?>