...
class User_EweiShopV2Page extends SnsMobilePage 
{
	public function main() 
	{
		...
		if (empty($member)) 
		{
			show_message(''.$this->lang['lang_plugin_sns_core_mobile_user_0'].'!', '', 'error');
		}
		$openid = $member['openid'];
		$level = array('levelname' => (empty($set['levelname']) ? ''.$this->lang['lang_plugin_sns_core_mobile_user_1'].'' : $set['levelname']), 'color' => (empty($set['levelcolor']) ? '#333' : $set['levelcolor']), 'bg' => (empty($set['levelbg']) ? '#eee' : $set['levelbg']));
	...
	public function boards() 
	{
		...
		if (empty($id)) 
		{
			if (!($this->islogin)) 
			{
				...
				$loginurl = mobileUrl('account/login', array('backurl' => urlencode(base64_encode($url))));
				$this->message(''.$this->lang['lang_plugin_sns_core_mobile_user_2'].'!', $url, 'error');
			}
			$member = $this->model->getMember($_W['openid']);
		}
		else 
		...
		if (empty($member)) 
		{
			show_message(''.$this->lang['lang_plugin_sns_core_mobile_user_3'].'!', '', 'error');
		}
		$openid = $member['openid'];
		include $this->template();
	}
	public function get_boards() 
	{
		...
		if (empty($id)) 
		{
			if (!($this->islogin)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_user_4'].'!');
			}
			$member = $this->model->getMember($_W['openid']);
		}
		else 
		...
		if (empty($member)) 
		{
			show_message(''.$this->lang['lang_plugin_sns_core_mobile_user_5'].'!', '', 'error');
		}
		$openid = $member['openid'];
	...
	public function posts() 
	{
		...
		if (empty($id)) 
		{
			if (!($this->islogin)) 
			{
				...
				$loginurl = mobileUrl('account/login', array('backurl' => urlencode(base64_encode($url))));
				$this->message(''.$this->lang['lang_plugin_sns_core_mobile_user_6'].'!', $url, 'error');
			}
			$member = $this->model->getMember($_W['openid']);
		}
		else 
		...
		if (empty($member)) 
		{
			show_message(''.$this->lang['lang_plugin_sns_core_mobile_user_7'].'!', '', 'error');
		}
		$openid = $member['openid'];
		include $this->template();
	}
	public function get_posts() 
	{
		...
		if (empty($id)) 
		{
			if (!($this->islogin)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_user_8'].'!');
			}
			$member = $this->model->getMember($_W['openid']);
		}
		else 
	...
	public function replys() 
	{
		...
		if (empty($id)) 
		{
			if (!($this->islogin)) 
			{
				...
				$loginurl = mobileUrl('account/login', array('backurl' => urlencode(base64_encode($url))));
				$this->message(''.$this->lang['lang_plugin_sns_core_mobile_user_9'].'!', $url, 'error');
			}
			$member = $this->model->getMember($_W['openid']);
		}
		else 
		...
		if (empty($member)) 
		{
			show_message(''.$this->lang['lang_plugin_sns_core_mobile_user_10'].'!', '', 'error');
		}
		$openid = $member['openid'];
		include $this->template();
	}
	public function get_replys() 
	{
		...
		if (!($this->islogin)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_user_11'].'!');
		}
		$openid = $_W['openid'];
		$pindex = max(1, intval($_GPC['page']));
		$psize = 10;
	...
	public function delete_reply() 
	{
		...
		if (!($this->islogin)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_user_12'].'!');
		}
		$id = intval($_GPC['id']);
		if (empty($id)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_user_13'].'!');
		}
		$post = $this->model->getPost($id);
		if (empty($post)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_user_14'].'!');
		}
		if ($post['openid'] !== $_W['openid']) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_user_15'].'TA'.$this->lang['lang_plugin_sns_core_mobile_user_16'].'!');
		}
	...
	public function submit_sign() 
	{
		...
		if (!($this->islogin)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_user_17'].'!');
		}
		$sign = trim($_GPC['sign']);
