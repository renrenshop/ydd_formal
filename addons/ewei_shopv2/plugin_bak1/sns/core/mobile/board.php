...
class Board_EweiShopV2Page extends SnsMobilePage 
{
	public function main() 
	{
		...
		if (empty($id)) 
		{
			$this->message(''.$this->lang['lang_plugin_sns_core_mobile_board_0'].'');
		}
		$board = $this->model->getBoard($id);
		if (empty($id)) 
		{
			$this->message(''.$this->lang['lang_plugin_sns_core_mobile_board_1'].'!');
		}
		$set = $this->set;
		if ($set['imagesnum'] == 0) 
		{
		...
		$followtip = ((empty($this->set['followtip']) ? ''.$this->lang['lang_plugin_sns_core_mobile_board_2'].'~' : $this->set['followtip']));
	...
	public function best() 
	{
		...
		if (empty($id)) 
		{
			$this->message(''.$this->lang['lang_plugin_sns_core_mobile_board_3'].'');
		}
		$board = $this->model->getBoard($id);
		if (empty($board)) 
		{
			$this->message(''.$this->lang['lang_plugin_sns_core_mobile_board_4'].'!');
		}
		$catelist = pdo_fetchall('SELECT id,name FROM ' . tablename('ewei_shop_sns_complaincate') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY displayorder asc');
	...
	public function relate() 
	{
		...
		if (empty($id)) 
		{
			$this->message(''.$this->lang['lang_plugin_sns_core_mobile_board_5'].'');
		}
		$board = $this->model->getBoard($id);
		if (empty($id)) 
		{
			$this->message(''.$this->lang['lang_plugin_sns_core_mobile_board_6'].'!');
		}
		$member = m('member')->getMember($_W['openid']);
		$isManager = $this->model->isManager($board['id']);
		$isSuperManager = $this->model->isSuperManager();
	...
	public function follow() 
	{
		if (!($this->islogin)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_board_7'].'');
		}
		global $_W;
		global $_GPC;
		$bid = intval($_GPC['bid']);
		...
		if (empty($bid)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_board_8'].'');
		}
		$board = $this->model->getBoard($bid);
		if (empty($board)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_sns_core_mobile_board_9'].'!');
		}
