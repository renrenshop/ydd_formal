...
class Index_EweiShopV2Page extends PluginMobilePage 
{
	...
	public function detail() 
	{
		...
		if (!(empty($id))) 
		{
			...
			if (empty($item)) 
			{
				$this->message(''.$this->lang['lang_plugin_qa_core_mobile_index_0'].'!');
			}
			$item['content'] = iunserializer($item['content']);
			$item['content'] = htmlspecialchars_decode($item['content']);
			$item['content'] = m('ui')->lazy($item['content']);
	...
	public function question() 
	{
		...
		if (!(empty($cate))) 
		{
			...
			if (empty($category)) 
			{
				$this->message(''.$this->lang['lang_plugin_qa_core_mobile_index_1'].'!');
			}
		}
		if (!(empty($set['share']))) 
		{
	...
	public function category() 
	{
		...
		if (empty($category)) 
		{
			$this->message(''.$this->lang['lang_plugin_qa_core_mobile_index_2'].'!');
		}
		$set = $this->model->getSet();
		if (!(empty($set['share']))) 
		{
			$_W['shopshare'] = array('title' => ''.$this->lang['lang_plugin_qa_core_mobile_index_3'].'', 'imgUrl' => tomedia($_W['shopset']['shop']['logo']), 'desc' => m('plugin')->getName('qa'), 'link' => mobileUrl('qa/category'));
			if (p('commission')) 
			{
				$commset = p('commission')->getSet();
				if (!(empty($commset['level']))) 
