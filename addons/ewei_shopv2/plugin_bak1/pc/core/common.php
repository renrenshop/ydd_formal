...
function fenye($total, $pageIndex, $pageSize = 15, $url = '', $context = array('before' => 5, 'after' => 4, 'ajaxcallback' => '')) 
{
	...
	if (1 < $pdata['cindex']) 
	{
		$html .= '<li><a ' . $pdata['faa'] . ' class="demo"><span>'.$this->lang['lang_plugin_pc_core_common_0'].'</span></a></li>';
		$html .= '<li><a ' . $pdata['paa'] . ' class="demo"><span>&laquo;'.$this->lang['lang_plugin_pc_core_common_1'].'</span></a></li>';
	}
	...
	{
		$html .= '<li><a ' . $pdata['naa'] . ' class="demo"><span>'.$this->lang['lang_plugin_pc_core_common_2'].'&raquo;</span></a></li>';
		$html .= '<li><a ' . $pdata['laa'] . ' class="demo"><span>'.$this->lang['lang_plugin_pc_core_common_3'].'</span></a></li>';
	}
	$html .= '</ul></div>';
	return $html;
}
...
function get_menus($type = 0) 
{
	...
	if ($type == 0) 
	{
		...
		{
			echo "<li><a href='" . mobileUrl("pc") . "'>'.$this->lang['lang_plugin_pc_core_common_4'].'</a></li>
";
		}
	}
	if ($type == 1) 
...
function truncateCn($string, $length = 80, $etc = '...', $charset = 'utf-8') 
{
	...
	$string = str_replace(array(' ', ''.$this->lang['lang_plugin_pc_core_common_5'].'', "\t", "\n", "\r"), array('', '', '', '', ''), $string);
	...
	if ($charset == 'utf-8') 
	{
		$pa = '/[' . "\x1" . '-'.$this->lang['lang_plugin_pc_core_common_6'].']|[' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . ']|' . "?" . '[' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . ']|[' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . ']|' . "?" . '[' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . ']|[' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . ']/';
	}
	else 
	{
		$pa = '/[' . "\x1" . '-'.$this->lang['lang_plugin_pc_core_common_7'].']|[' . "?" . '-' . "?" . '][' . "?" . '-' . "?" . ']/';
	}
