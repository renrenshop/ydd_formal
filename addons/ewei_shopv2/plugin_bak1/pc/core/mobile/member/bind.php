...
class Bind_EweiShopV2Page extends PcMobileLoginPage 
{
	...
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_bind_0'].'');
			}
			...
			if (!(empty($bind))) 
			{
				...
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_bind_1'].'');
				}
				else if (!(empty($member2)) && ($member2['id'] == $member['id']) && $member['mobileverify']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_bind_2'].'');
				}
			}
			else if (!(empty($member2)) && ($member2['id'] != $member['id']) && empty($confirm)) 
			{
				...
				{
					show_json(-1, ''.$this->lang['lang_plugin_pc_core_mobile_member_bind_3'].'<br>'.$this->lang['lang_plugin_pc_core_mobile_member_bind_4'].'');
				}
				show_json(-1, "'.$this->lang['lang_plugin_pc_core_mobile_member_bind_5'].'<br>'.$this->lang['lang_plugin_pc_core_mobile_member_bind_6'].'");
			}
			...
			if (!(empty($member2))) 
			{
				if (strexists($member2['openid'], 'sns_wx_') || strexists($member2['openid'], 'sns_qq_') || strexists($member2['openid'], 'wap_user_')) 
				{
					...
					{
						m('member')->setCredit($member['openid'], 'credit1', abs($member2['credit1']), 'WAP'.$this->lang['lang_plugin_pc_core_mobile_member_bind_7'].' +' . $member2['credit1']);
					}
					if (0 < $member2['credit2']) 
					{
						m('member')->setCredit($member['openid'], 'credit2', abs($member2['credit2']), 'WAP'.$this->lang['lang_plugin_pc_core_mobile_member_bind_8'].' +' . $member2['credit2']);
					}
		...
		$ice_menu_array = array( array('menu_key' => 'info', 'menu_name' => ''.$this->lang['lang_plugin_pc_core_mobile_member_bind_9'].'', 'menu_url' => mobileUrl('pc.member.info')), array('menu_key' => 'index', 'menu_name' => ''.$this->lang['lang_plugin_pc_core_mobile_member_bind_10'].'', 'menu_url' => mobileUrl('pc.member.bind')) );
