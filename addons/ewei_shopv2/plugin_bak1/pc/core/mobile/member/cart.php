...
class Cart_EweiShopV2Page extends PcMobileLoginPage 
{
	public function main() 
	{
		...
		foreach ($list as &$g ) 
		{
			...
			$g['unit'] = ((empty($data['unit']) ? ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_0'].'' : $data['unit']));
			if (empty($g['selected'])) 
			{
				$ischeckall = false;
			}
		...
		$nav_link_list = array( array('link' => mobileUrl('pc'), 'title' => ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_1'].''), array('link' => mobileUrl('pc.member'), 'title' => ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_2'].''), array('title' => ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_3'].'') );
		$ice_menu_array = array( array('menu_key' => 'index', 'menu_name' => ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_4'].'', 'menu_url' => mobileUrl('pc.member.cart')) );
		$merch_plugin = p('merch');
		$merch_data = m('common')->getPluginset('merch');
	...
	public function update() 
	{
		...
		if (empty($data)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_5'].'');
		}
		...
		if (empty($goods)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_6'].'');
		}
	...
	public function add() 
	{
		...
		if (empty($goods)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_7'].'');
		}
		$member = m('member')->getMember($_W['openid']);
		if (!(empty($_W['shopset']['wap']['open'])) && !(empty($_W['shopset']['wap']['mustbind'])) && empty($member['mobileverify'])) 
		{
			show_json(0, array('message' => ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_8'].'', 'url' => mobileUrl('member/bind', NULL, true)));
		}
		if (($goods['isverify'] == 2) || ($goods['type'] == 2) || ($goods['type'] == 3)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_9'].'<br>'.$this->lang['lang_plugin_pc_core_mobile_member_cart_10'].'');
		}
	...
	public function remove() 
	{
		...
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_11'].'');
		}
	...
	public function tofavorite() 
	{
		...
		if (empty($ids) || !(is_array($ids))) 
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_member_cart_12'].'');
		}
		foreach ($ids as $id ) 
		{
