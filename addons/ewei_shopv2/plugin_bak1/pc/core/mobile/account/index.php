...
class Index_EweiShopV2Page extends PcMobilePage 
{
	...
	public function login() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($member)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_0'].'');
			}
			if (md5($pwd . $member['salt']) !== $member['pwd']) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_1'].'');
			}
			m("account")->setLogin($member);
			show_json(1, "'.$this->lang['lang_plugin_pc_core_mobile_account_index_2'].'");
		}
		$set = $this->getWapSet();
		$backurl = '';
		if (!(empty($_GPC['backurl']))) 
	...
	protected function rf($type) 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($mobile)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_3'].'');
			}
			if (empty($verifycode)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_4'].'');
			}
			if (empty($pwd)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_5'].'');
			}
			...
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_6'].'!');
			}
			$member = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('ewei_shop_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
			if (empty($type)) 
			{
				if (!(empty($member))) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_7'].', '.$this->lang['lang_plugin_pc_core_mobile_account_index_8'].'');
				}
			...
			else 
			{
				if (empty($member)) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_9'].'');
				}
				$salt = m('account')->getSalt();
			...
			unset($_SESSION[$key]);
			show_json(1, (empty($type) ? ''.$this->lang['lang_plugin_pc_core_mobile_account_index_10'].'' : ''.$this->lang['lang_plugin_pc_core_mobile_account_index_11'].''));
		}
	...
	public function verifycode() 
	{
		...
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_12'].'');
		}
		$data = m('common')->getSysset('wap');
		$sms_id = $data[$temp];
		if (empty($sms_id)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_13'].'(NOSMSID)');
		}
		$key = '__ewei_shop_member_verifycodesession_' . $_W['uniacid'] . '_' . $mobile;
		@session_start();
		$code = random(5, true);
		$shopname = $_W['shopset']['shop']['name'];
		$ret = com('sms')->send($mobile, $sms_id, array(''.$this->lang['lang_plugin_pc_core_mobile_account_index_14'].'' => $code, ''.$this->lang['lang_plugin_pc_core_mobile_account_index_15'].'' => (!(empty($shopname)) ? $shopname : ''.$this->lang['lang_plugin_pc_core_mobile_account_index_16'].'')));
		if ($ret['status']) 
		{
			$_SESSION[$key] = $code;
			$_SESSION['verifycodesendtime'] = time();
			show_json(1, "'.$this->lang['lang_plugin_pc_core_mobile_account_index_17'].'");
		}
