...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	public function main() 
	{
		...
		$param_title = $set['shop']['name'] . ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_0'].'';
	...
	public function complete() 
	{
		...
		if (!(empty($ispeerpay))) 
		{
			...
			if (($ispeerpay['peerpay_type'] == 0) && ($ispeerpay['peerpay_realprice'] != $peerpay)) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_1'].'');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_2'].'', mobileUrl('pc.order'));
				}
			}
			else if (($ispeerpay['peerpay_type'] == 1) && !(empty($ispeerpay['peerpay_selfpay'])) && ($ispeerpay['peerpay_selfpay'] < $peerpay) && (0 < floatval($ispeerpay['peerpay_selfpay']))) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_3'].'');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_4'].'', mobileUrl('pc.order'));
				}
			}
			if ($peerpay <= 0) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_5'].'');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_6'].'', mobileUrl('pc.order'));
				}
			}
		...
		if (empty($orderid)) 
		{
			if ($_W['ispost']) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_7'].'');
			}
			else 
			{
				$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_8'].'', mobileUrl('pc.order'));
			}
		}
		$set = m('common')->getSysset(array('shop', 'pay'));
		$set['pay']['weixin'] = ((!(empty($set['pay']['weixin_sub'])) ? 1 : $set['pay']['weixin']));
		...
		if (0 < $order['ispackage']) 
		{
			...
			if (empty($package)) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_9'].'');
			}
			if (time() < $package['starttime']) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_10'].'');
			}
			if ($package['endtime'] < time()) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_11'].'');
			}
		}
		if (empty($order)) 
		{
			if ($_W['ispost']) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_12'].'');
			}
			else 
			{
				$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_13'].'', mobileUrl('pc.order'));
			}
		}
		$type = $_GPC['type'];
		if (!(in_array($type, array('wechat', 'alipay', 'credit', 'cash')))) 
		{
			if ($_W['ispost']) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_14'].'');
			}
			else 
			{
				$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_15'].'', mobileUrl('pc.order'));
			}
		}
		$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'ewei_shopv2', ':tid' => $order['ordersn']));
		if (empty($log) && empty($ispeerpay)) 
		{
			if ($_W['ispost']) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_16'].','.$this->lang['lang_plugin_pc_core_mobile_order_pay_17'].'!');
			}
			else 
			{
				$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_18'].','.$this->lang['lang_plugin_pc_core_mobile_order_pay_19'].'!', mobileUrl('pc.order'));
			}
		}
		...
		foreach ($order_goods as $data ) 
		{
			if (empty($data['status']) || !(empty($data['deleted']))) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, $data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_20'].'!');
				}
				else 
				{
					$this->message($data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_21'].'!', mobileUrl('pc.order'));
				}
			}
			$unit = ((empty($data['unit']) ? ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_22'].'' : $data['unit']));
			...
			if ($data['seckill']) 
			{
				if (empty($seckillinfo) || ($seckillinfo['status'] != 0) || ($seckillinfo['endtime'] < time())) 
				{
					if ($_W['ispost']) 
					{
						show_json(0, $data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_23'].'!');
					}
					else 
					{
						$this->message($data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_24'].'!', mobileUrl('pc.order'));
					}
				}
			}
			...
			else 
			{
				if (0 < $data['minbuy']) 
				{
					if ($data['buycount'] < $data['minbuy']) 
					{
						if ($_W['ispost']) 
						{
							show_json(0, $data['title'] . '<br/> ' . $data['min'] . $unit . ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_25'].'!', mobileUrl('pc.order'));
						}
						else 
						{
							$this->message($data['title'] . '<br/> ' . $data['min'] . $unit . ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_26'].'!', mobileUrl('pc.order'));
						}
					}
				}
				if (0 < $data['maxbuy']) 
				{
					if ($data['maxbuy'] < $data['buycount']) 
					{
						if ($_W['ispost']) 
						{
							show_json(0, $data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_27'].' ' . $data['maxbuy'] . $unit . '!');
						}
						else 
						{
							$this->message($data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_28'].' ' . $data['maxbuy'] . $unit . '!', mobileUrl('pc.order'));
						}
					}
				}
				if (0 < $data['usermaxbuy']) 
				{
					...
					if ($data['usermaxbuy'] <= $order_goodscount) 
					{
						if ($_W['ispost']) 
						{
							show_json(0, $data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_29'].' ' . $data['usermaxbuy'] . $unit);
						}
						else 
						{
							$this->message($data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_30'].' ' . $data['usermaxbuy'] . $unit, mobileUrl('pc.order'));
						}
					}
				}
				if ($data['istime'] == 1) 
				{
					if (time() < $data['timestart']) 
					{
						if ($_W['ispost']) 
						{
							show_json(0, $data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_31'].'!');
						}
						else 
						{
							$this->message($data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_32'].'!', mobileUrl('pc.order'));
						}
					}
					if ($data['timeend'] < time()) 
					{
						if ($_W['ispost']) 
						{
							show_json(0, $data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_33'].'!');
						}
						else 
						{
							$this->message($data['title'] . '<br/> '.$this->lang['lang_plugin_pc_core_mobile_order_pay_34'].'!', mobileUrl('pc.order'));
						}
					}
				}
				if ($data['buylevels'] != '') 
				{
					...
					if (!(in_array($member['level'], $buylevels))) 
					{
						if ($_W['ispost']) 
						{
							show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_35'].'<br/>' . $data['title'] . '!');
						}
						else 
						{
							$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_36'].'<br/>' . $data['title'] . '!', mobileUrl('pc.order'));
						}
					}
				}
				if ($data['buygroups'] != '') 
				{
					...
					if (!(in_array($member['groupid'], $buygroups))) 
					{
						if ($_W['ispost']) 
						{
							show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_37'].'<br/>' . $data['title'] . '!');
						}
						else 
						{
							$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_38'].'<br/>' . $data['title'] . '!', mobileUrl('pc.order'));
						}
					}
				}
			}
			if ($data['totalcnf'] == 1) 
			{
					...
					if (!(empty($option))) 
					{
						if ($option['stock'] != -1) 
						{
							if (empty($option['stock'])) 
							{
								if ($_W['ispost']) 
								{
									show_json(0, $data['title'] . '<br/>' . $option['title'] . ' '.$this->lang['lang_plugin_pc_core_mobile_order_pay_39'].'!');
								}
								else 
								{
									$this->message($data['title'] . '<br/>' . $option['title'] . ' '.$this->lang['lang_plugin_pc_core_mobile_order_pay_40'].'!', mobileUrl('pc.order'));
								}
							}
						}
					}
				...
				else if ($data['stock'] != -1) 
				{
					if (empty($data['stock'])) 
					{
						if ($_W['ispost']) 
						{
							show_json(0, $data['title'] . '<br/>'.$this->lang['lang_plugin_pc_core_mobile_order_pay_41'].'!');
						}
						else 
						{
							$this->message($data['title'] . '<br/>'.$this->lang['lang_plugin_pc_core_mobile_order_pay_42'].'!', mobileUrl('pc.order'));
						}
					}
				}
			}
		...
		if (($type == 'cash') && empty($ispeerpay)) 
		{
			if (empty($set['pay']['cash'])) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_43'].'!');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_44'].'', mobileUrl('pc.order'));
				}
			}
		...
		if (!(empty($ispeerpay))) 
		{
			...
			if ($ispeerpay['peerpay_realprice'] < $total) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_45'].'');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_46'].'');
				}
			}
			$log['fee'] = $peerpay;
			$openid = $_W['openid'];
		...
		if ($type == 'credit') 
		{
			if (empty($set['pay']['credit']) && (0 < $ps['fee'])) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_47'].'!');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_48'].'', mobileUrl('pc.order'));
				}
			}
			if ($ps['fee'] < 0) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_49'].'');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_50'].'', mobileUrl('pc.order'));
				}
			}
			...
			if ($credits < $ps['fee']) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_51'].','.$this->lang['lang_plugin_pc_core_mobile_order_pay_52'].'');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_53'].','.$this->lang['lang_plugin_pc_core_mobile_order_pay_54'].'', mobileUrl('pc.order'));
				}
			}
			$fee = floatval($ps['fee']);
			$result = m('member')->setCredit($openid, 'credit2', -$fee, array($_W['member']['uid'], $_W['shopset']['shop']['name'] . ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_55'].'' . $fee));
			if (is_error($result)) 
			{
				if ($_W['ispost']) 
				{
		...
		if ($type == 'wechat') 
		{
			if (!(is_weixin()) && empty($_W['shopset']['wap']['open'])) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, (is_h5app() ? 'APP'.$this->lang['lang_plugin_pc_core_mobile_order_pay_56'].'' : ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_57'].'!'));
				}
				else 
				{
					$this->message((is_h5app() ? 'APP'.$this->lang['lang_plugin_pc_core_mobile_order_pay_58'].'' : ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_59'].'!'), mobileUrl('pc.order'));
				}
			}
			if ((empty($set['pay']['weixin']) && empty($set['pay']['weixin_jie']) && is_weixin()) || (empty($set['pay']['app_wechat']) && is_h5app())) 
			{
				if ($_W['ispost']) 
				{
					show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_60'].'!');
				}
				else 
				{
					$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_61'].'!', mobileUrl('pc.order'));
				}
			}
			...
			if (!(empty($ispeerpay))) 
			{
				...
				show_json(1, "'.$this->lang['lang_plugin_pc_core_mobile_order_pay_62'].'");
			}
			...
			if ($_W['ispost']) 
			{
				show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_63'].','.$this->lang['lang_plugin_pc_core_mobile_order_pay_64'].'!');
				return;
			}
			$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_65'].','.$this->lang['lang_plugin_pc_core_mobile_order_pay_66'].'!', mobileUrl('pc.order'));
		}
	}
	public function success() 
	{
		...
		if (empty($orderid)) 
		{
			$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_pay_67'].'', mobileUrl('pc.order'), 'error');
		}
		$ispeerpay = m('order')->checkpeerpay($orderid);
		if (!(empty($ispeerpay))) 
		{
			...
			$openid = pdo_fetchcolumn('select openid from ' . tablename('ewei_shop_order') . ' where id=:orderid and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':uniacid' => $uniacid));
			$order['price'] = ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_68'].'';
	...
	public function peerpayshare() 
	{
		...
		$goods = pdo_fetch('select * from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $ordergoods['goodsid'], ':uniacid' => $uniacid));
		$_W['shopshare'] = array('title' => ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_69'].'', 'imgUrl' => tomedia($goods['thumb']), 'desc' => $peerpay['peerpay_message'], 'link' => mobileUrl('pc.order/pay/peerpaydetail', array('id' => $peerid), 1));
		include $this->template();
	}
	public function peerpaydetail() 
	{
		...
		$_W['shopshare'] = array('title' => ''.$this->lang['lang_plugin_pc_core_mobile_order_pay_70'].'', 'imgUrl' => tomedia($ordergoods['thumb']), 'desc' => $peerpay['peerpay_message'], 'link' => mobileUrl('pc.order/pay/peerpaydetail', array('id' => $peerid), 1));
		include $this->template();
	}
}
?>