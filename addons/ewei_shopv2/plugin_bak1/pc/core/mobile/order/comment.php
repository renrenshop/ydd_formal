...
class Comment_EweiShopV2Page extends MobileLoginPage 
{
	public function __construct() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_comment_0'].'!', '', 'error');
		}
	}
	public function main() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_comment_1'].'!', mobileUrl('order/detail', array('id' => $orderid)));
		}
		if (2 <= $order['iscomment']) 
		{
			$this->message(''.$this->lang['lang_plugin_pc_core_mobile_order_comment_2'].'!', mobileUrl('order/detail', array('id' => $orderid)));
		}
	...
	public function submit() 
	{
		...
		if (empty($order)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_comment_3'].'');
		}
		$member = m('member')->getMember($openid);
		$comments = $_GPC['comments'];
		if (!(is_array($comments))) 
		{
			show_json(0, ''.$this->lang['lang_plugin_pc_core_mobile_order_comment_4'].'!');
		}
		$trade = m('common')->getSysset('trade');
