	...
	$.addFlex = function(t, p) {
		...
		if (p.colModel) {
			...
			for (i = 0; i < p.colModel.length; i++) {
				...
				if (cm.className) th.className = cm.className;
		...
		if (p.buttons) {
			...
			for (i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					if (btn.bclass) $('div', btnDiv).addClass(btn.bclass).attr('title', btn.title);
		...
		if (p.columnControl && $('th', g.hDiv).length) {
			...
			if ($.browser.msie && $.browser.version < 7.0) $('li', g.nDiv).hover(
			...
			$(g.nBtn).addClass('nBtn').html('<span class="hBtn"><i class="fa fa-cogs"></i></span>').hover(function() {
				$(g.nDiv).show();
