...
//检索
$(function() { /*	分类检索	*/
	retrieval("cate", false); /*	品质检索	*/
	retrieval("quality", false); /*	地区检索	*/
	retrieval("area", false); /*	价格检索	*/
	retrieval("price", true); /*	价格搜索	*/
	...
	function rlprice() {
		if ($("#start_input").val()) $("#start_input").remove();
		if ($("#end_input").val()) $("#end_input").remove();
	}
	//共享函数：搜索
	...
	function scstart() {
		if ($("#price_input").val()) $("#price_input").remove();
	}
...
$(function() {
	...
	$("a[keycate=" + cate + "]").parent().addClass("current"); /*	品质选择	*/
	var quality = $("#quality_input").val();
	$("a[keyquality=" + quality + "]").parent().addClass("current"); /*	地区选择	*/
	var area = $("#area_input").val();
	$("a[keyarea=" + area + "]").parent().addClass("current"); /*	时间选择	*/
