	...
	ImageZoom.prototype = {
		...
		buildDOM: function() {
			...
			$.each(ops.items, function(i) {
				controllerHTML += ['<li><a href="javascript:;" data-index="', i, '"><img src="', ops.basePath + this.levelA, '" height="', ops.levelASize[0], '" width="', ops.levelASize[0], '" alt="" /></a></li>'].join('');
			});
			controllerHTML += '</ul>';
			...
			if (!this.draging && (index !== prevIndex || level !== prevLevel) && ops.onBeforeZoom.call(this, index, level, prevIndex, prevLevel) !== false) {
				...
				if (level >= 3) {
					...
					offset = [pointXY[0] - galleryOffset.left, pointXY[1] - galleryOffset.top],
						...
						xy = this.getLimitXY(position[0] - size[0] * ratio[0] + offset[0], position[1] - size[1] * ratio[1] + offset[1]);
		...
		getLimitXY: function(x, y, level) {
			var ops = this.ops,
				size = ops['level' + (levelMap[level] || 'D') + 'Size'];
			...
			offset = [0, 0],
				...
				moveHandler = function(e) {
					...
					panelOffset = self.galleryPanel.offset(),
						xy = self.getLimitXY(e.pageX - panelOffset.left - offset[0], e.pageY - panelOffset.top - offset[1]);
