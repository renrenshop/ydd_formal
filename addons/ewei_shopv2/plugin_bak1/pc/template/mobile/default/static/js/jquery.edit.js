$(document).ready(function() {
	...
	$('span[nc_type="inline_edit"]').click(function() {
		...
		$('<input type="text">').attr({
			value: s_value
		}).insertAfter($(this)).focus().select().keyup(function(event) {
			if (event.keyCode == 13) {
				if (req) {
	...
	$('span[nc_type="inline_edit_textarea"]').click(function() {
		...
		$('<textarea>').attr({
			value: s_value
		}).appendTo($(this).parent()).focus().select().keyup(function(event) {
			if (event.keyCode == 13) {
				if (req) {
					if (!required($(this).attr('value'), s_value, $(this))) {
						return;
	...
	$('a[nc_type="inline_edit"]').click(function() {
		...
		$.get('index.php?act=' + act + '&op=ajax', {
			...
			if (data == 'true') {
				if (i_val == 0) {
					$('a[fieldid="' + i_id + '"][fieldname="' + i_name + '"]').attr({
	...
	//给列表有排序行为的列添加鼠标手型效果
	$('span[nc_type="order_by"]').hover(function() {
		$(this).css({
			cursor: 'pointer'
		});
	}, function() {});
...
(function($) {
	$.fn.inline_edit = function(options) {
		...
		function onClick() {
			...
			$('<input type="text">').insertAfter($(this)).focus().select().val(old_value).blur(function() {
				var new_value = $(this).attr("value");
				if (new_value != '') {
					$.get('index.php?act=' + settings.act + '&op=' + settings.op + '&branch=ajax', {
						id: column_id,
						value: new_value
					}, function(data) {
						data = $.parseJSON(data);
		...
		function onClick() {
			...
			$btn_submit.click(function() {
				...
				if (new_value !== '' && new_value !== old_value) {
					$.post('index.php?act=' + settings.act + '&op=' + settings.op, {
						id: column_id,
						value: new_value
					}, function(data) {
						data = $.parseJSON(data);
