  ...
  $(function() {
  	...
  	$(".ncap-channel-category").sortable({
  		items: 'dl'
  	});
  	$(".ncap-channel-category ul").sortable({
  		items: 'li'
  	});

  	slide_obj = $("#upload_slide_form"); //初始化焦点大图区数据
  	upload_obj = $("#upload_slide");
  	slide_obj.find("ul").sortable({
  		items: 'li'
  	});

  	adv_obj = $("#upload_adv_form"); //初始化三张联动区数据
  	adv_obj.find(".focus-trigeminy").sortable({
  		items: 'div[focus_id]'
  	});
  	adv_obj.find("ul").sortable({
  		items: 'li'
  	});
  ...
  function add_slide() { //增加图片
  	for (var i = 1; i <= slide_max; i++) { //防止数组下标重复
  		if (slide_obj.find("li[slide_id='" + i + "']").size() == 0) { //编号不存在时添加
  			...
  			var add_html = '';
  			add_html = '<li slide_id="' + i + '" title="可上下拖拽更改显示顺序"><div class="title"><h4></h4><a class="ncap-btn-mini del" href="JavaScript:del_slide(' + i + ');"><i class="fa fa-trash"></i>删除</a></div><div class="focus-thumb" onclick="select_slide(' + i + ');" title="点击编辑选中区域内容"><img src="' + ADMIN_TEMPLATES_URL + '/images/picture.gif" /></div>' + text_input + '</li>';
  ...
  function add_adv() { //增加
  	for (var i = 1; i <= adv_max; i++) { //防止数组下标重复
  		if (adv_obj.find("div[focus_id='" + i + "']").size() == 0) { //编号不存在时添加
  			var add_html = '';
  			var text_type = '图片调用';
  			add_html = '<div focus_id="' + i + '" class="focus-trigeminy-group" title="可上下拖拽更改显示顺序"><div class="title"><h4></h4>' + '<a class="ncap-btn-mini del" href="JavaScript:del_adv(' + i + ');"><i class="fa fa-trash"></i>删除</a></div><ul></ul></div>';
