	...
	$(function() {
		...
		function() {
			...
			$.getJSON('index.php?act=store_setting&op=dorp_img', {
				file_id: file_id,
				img_src: img_src
			}, function(data) {
	...
	function ajaxFileUpload(url, id, file_id) {
		$('div[nctype="' + id + '"]').find('i').remove().end().find('img').remove().end().prepend('<img nctype="' + id + '" scr="' + SHOP_TEMPLATES_URL + '/images/loading.gif">');
