...
$.fn.tree = function(settings) {
	...
	return $(this).each(function() {
		if (!$(this).parents('.tree').length) {
			...
			tree.find('li:has(ul)').attr('aria-expanded', 'false').find('>i').addClass('tree-parent tree-parent-collapsed');

			//expanded at load		
			tree.find(o.expanded).attr('aria-expanded', 'true').find('>i').removeClass('tree-parent-collapsed').nextAll('ul').removeClass('tree-group-collapsed');
