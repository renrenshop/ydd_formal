define(['core', 'tpl', 'biz/member/cart'], function(core, tpl, cart) {
	...
	modal.open = function(params) {
		...
		if (modal.goodsid != params.goodsid) {
			...
			core.json('goods/picker', {
				...
				if (ret.status == 0) {
					FoxUI.toast.show(lang_js_pc_biz_goods_picker_0+'!');
					return
				}
				modal.followtip = '';
				modal.followurl = '';
				...
				if (modal.goods.unit == '') {
					modal.goods.unit = lang_js_pc_biz_goods_picker_1
				}
				modal.show()
			}, true, false)
		} else {
	...
	modal.init = function() {
		...
		$('.fui-number', modal.container.container).numbers({
			...
			minToast: "{min}" + modal.goods.unit + ""+lang_js_pc_biz_goods_picker_2+"",
			maxToast: ""+lang_js_pc_biz_goods_picker_3+"{max}" + modal.goods.unit,
		...
		$('.cartbtn', modal.container.container).unbind('click').click(function() {
			if (!modal.goods.canAddCart) {
				FoxUI.toast.show(lang_js_pc_biz_goods_picker_4+"<br>"+lang_js_pc_biz_goods_picker_5);
				return
			}
	...
	modal.show = function() {
		...
		if (modal.needlogin) {
			...
			FoxUI.confirm(""+lang_js_pc_biz_goods_picker_8+"", ""+lang_js_pc_biz_goods_picker_9+"", function() {
		...
		if (modal.mustbind) {
			FoxUI.alert(""+lang_js_pc_biz_goods_picker_10+"", ""+lang_js_pc_biz_goods_picker_11+"", function() {
	...
	modal.chooseSpec = function(obj, callback) {
		...
		if (selected.length == modal.specs.length) {
			...
			$.each(modal.options, function() {
				...
				if (specs == itemids.sort().join('_')) {
					var stock = this.stock == '-1' ? lang_js_pc_biz_goods_picker_12 : this.stock;
					$('.total', modal.container.container).html(stock);
					if (this.stock != '-1' && this.stock <= 0) {
						$('.confirmbtn', modal.container).show().addClass('disabled').html(lang_js_pc_biz_goods_picker_13);
						$('.cartbtn,.buybtn', modal.container).hide()
					} else {
						if (modal.params.showConfirm) {
							$('.confirmbtn', modal.container).removeClass('disabled').html(lang_js_pc_biz_goods_picker_14);
							$('.cartbtn,.buybtn', modal.container).hide()
						} else {
		...
		$('.info-titles', modal.container).html(lang_js_pc_biz_goods_picker_15+' ' + modal.params.titles);
		if (callback) {
	...
	modal.check = function() {
		...
		spec.each(function() {
			if ($(this).find('.spec-item.btn-danger').length <= 0) {
				FoxUI.toast.show(lang_js_pc_biz_goods_picker_16 + $(this).find('.title').html());
				selected = false;
				return false
			}
		});
		if (selected) {
			if (modal.option.stock != -1 && modal.option.stock <= 0) {
				FoxUI.toast.show(lang_js_pc_biz_goods_picker_17);
				return false
			}
			...
			if (modal.goods.maxbuy > 0 && num > modal.goods.maxbuy) {
				FoxUI.toast.show(lang_js_pc_biz_goods_picker_18+' ' + modal.goods.maxbuy + ' ' + modal.goods.unit);
				return false
			}
			if (modal.goods.minbuy > 0 && num < modal.goods.minbuy) {
				FoxUI.toast.show(modal.goods.minbuy + modal.goods.unit + lang_js_pc_biz_goods_picker_19);
				return false
			}
			return true
		}
