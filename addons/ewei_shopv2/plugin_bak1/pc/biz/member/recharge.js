define(['core', 'tpl'], function(core, tpl) {
	...
	modal.init = function(params) {
				...
				if ($.isNumber(money) && parseFloat(money) > 0) {
					if (modal.minimumcharge > 0) {
						if (parseFloat(money) < modal.minimumcharge) {
							FoxUI.toast.show(lang_js_pc_biz_member_recharge_0+ modal.minimumcharge +lang_js_pc_biz_member_recharge_1+'!');
							return
						} else {
							showpay = true
						}
						...
						require(['biz/sale/coupon/picker'], function(picker) {
							picker.show({
								...
								onCancel: function() {
									...
									$('#coupondiv').find('.fui-cell-label').html(lang_js_pc_biz_member_recharge_2);
									$('#coupondiv').find('.fui-cell-info').html('')
								},
								onSelected: function(data) {
									$('#coupondiv').find('.fui-cell-label').html(lang_js_pc_biz_member_recharge_3);
		...
		$(document).on('click', '#btn-wechat', function() {
			...
			var money = $('#money').val();
			if (money <= 0) {
				FoxUI.toast.show(lang_js_pc_biz_member_recharge_4+'0!');
				return
			}
			if (!$('#money').isNumber()) {
				FoxUI.toast.show(lang_js_pc_biz_member_recharge_5+'!');
				return
			}
						...
						if (res.err_msg == 'get_brand_wcpay_request:ok') {
							core.json('member/recharge/wechat_complete', {
								...
								if (pay_json.status == 1) {
									FoxUI.toast.show(lang_js_pc_biz_member_recharge_6+'!');
							...
							$('.btn-pay').removeAttr('submit');
							FoxUI.toast.show(lang_js_pc_biz_member_recharge_7)
						} else {
							core.json('member/recharge/submit', {
								type: 'wechat',
								money: money,
		...
		$(document).on('click', '#btn-alipay', function() {
			...
			if (money <= 0) {
				FoxUI.toast.show(lang_js_pc_biz_member_recharge_8+'0!');
				return
			}
			var money = $('#money').val();
			if (!$('#money').isNumber()) {
				FoxUI.toast.show(lang_js_pc_biz_member_recharge_9+'!');
				return
			}
			$('.btn-pay').attr('submit', 1);
			core.json('member/recharge/submit', {
