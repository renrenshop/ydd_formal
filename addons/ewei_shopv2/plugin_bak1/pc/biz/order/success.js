				...
				if (this.getStatus() == BMAP_STATUS_SUCCESS) {
					...
					$('.store-item').each(function() {
						...
						if (store_lng > 0 && store_lat > 0) {
							...
							location.html(lang_js_pc_biz_order_success_0+': ' + distance.toFixed(2) + "km").show()
						} else {
							$(this).data('distance', 999999999999999999);
							location.html(lang_js_pc_biz_order_success_1).show()
						}
						arr.push($(this))
					});
					...
					$('#nearStoreHtml').find('.location').html(location + "<span class='fui-label fui-label-danger'>"+lang_js_pc_biz_order_success_2+"</span> ");
					$(arr[0]).remove()
				}
			}, {
