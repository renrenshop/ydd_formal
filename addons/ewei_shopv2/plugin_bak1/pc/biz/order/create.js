define(['core', 'tpl', 'biz/plugin/diyform'], function(core, tpl, diyform) {
	...
	modal.init = function(params) {
		...
		$('#coupondiv').find('.fui-cell-label').html(lang_js_pc_biz_order_create_0);
		...
		if (number.length > 0) {
			var maxbuy = number.data('maxbuy') || 0,
				...
				unit = number.data('unit') || lang_js_pc_biz_order_create_1;
			number.numbers({
				max: maxbuy,
				min: minbuy,
				minToast: "{min}" + unit + ""+lang_js_pc_biz_order_create_2+"",
				maxToast: ""+lang_js_pc_biz_order_create_3+"{max}" + unit,
		...
		$('#coupondiv').unbind('click').click(function() {
			core.json('sale/coupon/util/query', {
				...
				if (rjson.result.coupons.length > 0) {
					...
					require(['biz/sale/coupon/picker'], function(picker) {
						picker.show({
							...
							onCancel: function() {
								...
								$('#coupondiv').find('.fui-cell-label').html(lang_js_pc_biz_order_create_4);
								$('#coupondiv').find('.fui-cell-info').html('');
								modal.calcCouponPrice()
							},
							onSelected: function(data) {
								...
								$('#coupondiv').find('.fui-cell-label').html(lang_js_pc_biz_order_create_5);
					...
					FoxUI.toast.show(lang_js_pc_biz_order_create_6+'!');
					modal.hideCoupon()
				}
			}, false, true)
		})
	...
	modal.submit = function(obj, token) {
		...
		var giftid = $("#giftid").val();
		if (giftid == '') {
			FoxUI.toast.show(lang_js_pc_biz_order_create_7);
			return
		}
		if (modal.params.mustbind) {
			FoxUI.alert(""+lang_js_pc_biz_order_create_8+"", ""+lang_js_pc_biz_order_create_9+"", function() {
		...
		if (modal.params.iscarry || modal.params.isverify || modal.params.isvirtual) {
			if (modal.params.iscarry && modal.storeid == 0) {
				FoxUI.toast.show(lang_js_pc_biz_order_create_10);
				return
			}
			if ($(':input[name=carrier_realname]').isEmpty() && $(':input[name=carrier_realname]').attr("data-set") == 0) {
				FoxUI.toast.show(lang_js_pc_biz_order_create_11);
				return
			}
			if ($(':input[name=carrier_mobile]').isEmpty() && $(':input[name=carrier_mobile]').attr("data-set") == 0) {
				FoxUI.toast.show(lang_js_pc_biz_order_create_12);
				return
			}
			if (!$(':input[name=carrier_mobile]').isMobile()) {
				FoxUI.toast.show(lang_js_pc_biz_order_create_13);
				return
			}
		} else {
			if (modal.params.addressid == 0) {
				FoxUI.toast.show(lang_js_pc_biz_order_create_14);
				return
			} else {
		...
		FoxUI.loader.show(lang_js_pc_biz_order_create_15+'...', 'icon icon-add');
