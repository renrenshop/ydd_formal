	...
	modal.init = function(params) {
		...
		$('.btn-submit').click(function() {
			...
			if (modal.params.iscomment == 0 && modal.params.level < 1) {
				FoxUI.toast.show(lang_js_pc_biz_order_comment_0);
				return
			}
			if ($('#comment').isEmpty()) {
				FoxUI.toast.show(lang_js_pc_biz_order_comment_1+'!');
				return
			}
			var default_images = [];
			...
			$(this).html(lang_js_pc_biz_order_comment_2+'...').attr('stop', 1);
			core.json('order/comment/submit', {
				...
				$('.btn-submit').removeAttr('stop').html(lang_js_pc_biz_order_comment_3);
