...
class Index_EweiShopV2Page extends PluginMobilePage 
{
	public function main() 
	{
		...
		{
			$this->message(''.$this->lang['lang_plugin_seckill_core_mobile_index_0'].' redis '.$this->lang['lang_plugin_seckill_core_mobile_index_1'].'', '', 'error');
			exit();
		}
		...
		if (empty($taskid)) 
		{
			...
			if (empty($taskid)) 
			{
				$this->message(''.$this->lang['lang_plugin_seckill_core_mobile_index_2'].'~');
				exit();
			}
		}
		...
		if (empty($task)) 
		{
			$this->message(''.$this->lang['lang_plugin_seckill_core_mobile_index_3'].'');
			exit();
		}
		$rooms = $this->model->getRooms($taskid);
		if (empty($rooms)) 
		{
			$this->message(''.$this->lang['lang_plugin_seckill_core_mobile_index_4'].'');
			exit();
		}
		$room = false;
		$roomindex = 0;
		...
		if (empty($room)) 
		{
			$this->message(''.$this->lang['lang_plugin_seckill_core_mobile_index_5'].'');
			exit();
		}
		$roomid = $room['id'];
		$timeid = 0;
	...
	public function get_goods() 
	{
		...
		if (empty($task)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_seckill_core_mobile_index_6'].'');
		}
		$room = $this->model->getRoomInfo($taskid, $roomid);
		if (empty($room)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_seckill_core_mobile_index_7'].'');
		}
		$time = false;
		$nexttime = false;
		...
		if (empty($time)) 
		{
			show_json(0, ''.$this->lang['lang_plugin_seckill_core_mobile_index_8'].'');
		}
		$currenttime = time();
