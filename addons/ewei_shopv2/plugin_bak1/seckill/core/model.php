...
class SeckillModel extends PluginModel 
{
				...
				foreach ($orders as $orderid ) 
				{
					...
					if (!(empty($o)) && ($o['status'] == 0)) 
					{
						if ($o['isparent'] == 0) 
						{
							...
							if (0 < $o['deductprice']) 
							{
								m('member')->setCredit($o['openid'], 'credit1', $o['deductcredit'], array('0', $_W['shopset']['shop']['name'] . ''.$this->lang['lang_plugin_seckill_core_model_0'].' '.$this->lang['lang_plugin_seckill_core_model_1'].': ' . $o['deductcredit'] . ' '.$this->lang['lang_plugin_seckill_core_model_2'].': ' . $o['deductprice'] . ' '.$this->lang['lang_plugin_seckill_core_model_3'].': ' . $o['ordersn']));
							}
						}
	...
	public function orderRefund($item) 
	{
		...
		if ($item['paytype'] == 1) 
		{
			m('member')->setCredit($item['openid'], 'credit2', $realprice, array(0, $shopset['name'] . ''.$this->lang['lang_plugin_seckill_core_model_4'].': ' . $realprice . ''.$this->lang['lang_plugin_seckill_core_model_5'].' '.$this->lang['lang_plugin_seckill_core_model_6'].': ' . $item['ordersn']));
			$result = true;
		}
		...
		if (0 < $credits) 
		{
			m('member')->setCredit($item['openid'], 'credit1', -$credits, array(0, $shopset['name'] . ''.$this->lang['lang_plugin_seckill_core_model_7'].': ' . $credits . ' '.$this->lang['lang_plugin_seckill_core_model_8'].': ' . $item['ordersn']));
		}
		if (0 < $item['deductcredit']) 
		{
			m('member')->setCredit($item['openid'], 'credit1', $item['deductcredit'], array('0', $shopset['name'] . ''.$this->lang['lang_plugin_seckill_core_model_9'].' '.$this->lang['lang_plugin_seckill_core_model_10'].': ' . $item['deductcredit'] . ' '.$this->lang['lang_plugin_seckill_core_model_11'].': ' . $item['deductprice'] . ' '.$this->lang['lang_plugin_seckill_core_model_12'].': ' . $item['ordersn']));
		}
		if (!(empty($refundtype))) 
		{
			if ($realprice < 0) 
	...
	public function checkBuy($seckillinfo, $title, $unit = '') 
	{
		if (empty($unit)) 
		{
			$unit = ''.$this->lang['lang_plugin_seckill_core_model_13'].'';
		}
		if (100 <= $seckillinfo['percent']) 
		{
			if (0 < $seckillinfo['notpay']) 
			{
				return error(-1, $title . '<br/> '.$this->lang['lang_plugin_seckill_core_model_14'].' '.$this->lang['lang_plugin_seckill_core_model_15'].' ' . $seckillinfo['notpay'] . ' ' . $unit . ''.$this->lang['lang_plugin_seckill_core_model_16'].', '.$this->lang['lang_plugin_seckill_core_model_17'].'~');
			}
			return error(-1, $title . '<br/> '.$this->lang['lang_plugin_seckill_core_model_18'].' !');
		}
		if (0 < $seckillinfo['totalmaxbuy']) 
		{
			if ($seckillinfo['totalmaxbuy'] <= $seckillinfo['selftotalcount']) 
			{
				...
				{
					return error(-1, $title . '<br/> '.$this->lang['lang_plugin_seckill_core_model_19'].' ' . $seckillinfo['totalmaxbuy'] . ' ' . $unit . ',  '.$this->lang['lang_plugin_seckill_core_model_20'].'' . $seckillinfo['selftotalnotpay'] . ''.$this->lang['lang_plugin_seckill_core_model_21'].'~');
				}
				return error(-1, $title . '<br/> '.$this->lang['lang_plugin_seckill_core_model_22'].' ' . $seckillinfo['totalmaxbuy'] . ' ' . $unit . ''.$this->lang['lang_plugin_seckill_core_model_23'].'!');
			}
		}
		if (0 < $seckillinfo['maxbuy']) 
		{
			if ($seckillinfo['maxbuy'] <= $seckillinfo['selfcount']) 
			{
				...
				{
					return error(-1, $title . '<br/> '.$this->lang['lang_plugin_seckill_core_model_24'].' ' . $seckillinfo['maxbuy'] . ' ' . $unit . ',  '.$this->lang['lang_plugin_seckill_core_model_25'].'' . $seckillinfo['selfnotpay'] . ''.$this->lang['lang_plugin_seckill_core_model_26'].'~');
				}
				return error(-1, $title . '<br/> '.$this->lang['lang_plugin_seckill_core_model_27'].' ' . $seckillinfo['maxbuy'] . ' ' . $unit . ''.$this->lang['lang_plugin_seckill_core_model_28'].'!');
			}
		}
		return true;
	}
		...
		foreach ($times as $time ) 
		{
			...
			foreach ($goods as $g ) 
			{
				if (in_array($g['goodsid'], $goodsids) && ($g['roomid'] != $roomid)) 
				{
					...
					{
						$error['goods-' . $g['goodsid']] = ''.$this->lang['lang_plugin_seckill_core_model_29'].'&lt;span class=\'text text-danger\'&gt;'.$this->lang['lang_plugin_seckill_core_model_30'].'' . $goodstitle . ''.$this->lang['lang_plugin_seckill_core_model_31'].'&lt;/span&gt;'.$this->lang['lang_plugin_seckill_core_model_32'].'&lt;a href=\'' . $url . '\' target=\'_blank\'&gt;'.$this->lang['lang_plugin_seckill_core_model_33'].'' . $room['title'] . ''.$this->lang['lang_plugin_seckill_core_model_34'].'&lt;/a&gt;'.$this->lang['lang_plugin_seckill_core_model_35'].' &lt;span class=\'text text-danger\'&gt;' . $time['time'] . '&lt;/span&gt; '.$this->lang['lang_plugin_seckill_core_model_36'].'';
					}
				}
			}
		}
