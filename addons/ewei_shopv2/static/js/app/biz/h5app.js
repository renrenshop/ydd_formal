define(["core", "ydb"], function(core) {
    ...
    modal.initHM = function() {
        ...
        $(".head-menu").find("nav").unbind("click").click(function() {
            ...
            if (type == "exitapp") {
                FoxUI.confirm(lang_js_biz_h5app_0+"？", function() {
                    modal.YDB.ExitApp()
                })
            }
            if (type == "browser") {}
    ...
    modal.initShare = function() {
        if (!modal.share) {
            FoxUI.alert(lang_js_biz_h5app_1);
            return
        }
    ...
    modal.initSNS = function() {
        $(document).on('click', ".btn-sns", function() {
            ...
            if (sns == "wx") {
                FoxUI.toast.show(lang_js_biz_h5app_2);
                ...
                if (sns == "qq") {
                    FoxUI.toast.show(lang_js_biz_h5app_3+"QQ");
    ...
    window.appPay = modal.appPay = function(app, ordersn, money, status, callback) {
        ...
        if (!modal.payinfo) {
            FoxUI.toast.show(lang_js_biz_h5app_4+"(0)");
            return
        }
        if (!ordersn || money <= 0) {
            FoxUI.toast.show(lang_js_biz_h5app_5+"(1)");
            return
        }
        if (app == 'wechat') {
            if (!modal.payinfo.mcname) {
                FoxUI.toast.show(lang_js_biz_h5app_6+"(2)");
                return
            }
            FoxUI.toast.show(lang_js_biz_h5app_7);
            modal.YDB.SetWxpayInfo(modal.payinfo.mcname, lang_js_biz_h5app_8 + ordersn, money, ordersn, modal.payinfo.attach)
        }
        if (app == 'alipay') {
            if (!modal.payinfo.aliname) {
                FoxUI.toast.show(lang_js_biz_h5app_9+"(2)");
                return
            }
            FoxUI.toast.show(lang_js_biz_h5app_10);
