    ...
    var CouponPicker = function(params) {
        ...
        self.show = function() {
            ...
            $('.coupon-picker').find('.ling').click(function() {
                ...
                btn.attr('oldhtml', btn.html()).html(lang_js_biz_sale_coupon_couponpicker_0+'..').attr('submitting', 1);
                core.json('goods/detail/pay', {
                    id: couponid
                }, function(ret) {
                    if (ret.status <= 0) {
                        btn.html(btn.attr('oldhtml')).removeAttr('oldhtml').removeAttr('submitting');
                        $(".fui-message-popup .btn-danger").text(lang_js_biz_sale_coupon_couponpicker_1);
                        modal.paying = false;
                        modal.picker.close();
                        FoxUI.toast.show(ret.result.message)
                    } else {
                        FoxUI.toast.show(lang_js_biz_sale_coupon_couponpicker_2);
                        btn.html(btn.attr('oldhtml')).removeAttr('oldhtml').removeAttr('submitting');
                        modal.picker.close()
                    }
                }, true, true)
