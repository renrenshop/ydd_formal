define(['core', 'tpl'], function(core, tpl) {
    ...
    modal.init = function(params) {
        ...
        $('#rtype').change(function() {
            ...
            if (rtype == 2) {
                $('.r-group').hide();
                $('.re-g').html(lang_js_biz_order_refund_0)
            } else {
                $('.r-group').show();
                $('.re-g').html(lang_js_biz_order_refund_1)
            }
        });
        ...
        $('.btn-submit').click(function() {
            ...
            if ($('#rtype').val() != 2) {
                if (!$('#price').isNumber()) {
                    FoxUI.toast.show(lang_js_biz_order_refund_2+'!');
                    return
                }
                if ($('#price').val() < 0) {
            ...
            $(this).attr('stop', 1).html(lang_js_biz_order_refund_3+'...');
            core.json('order/refund/submit', {
                ...
                $('.btn-submit').removeAttr('stop').html(lang_js_biz_order_refund_4);
        ...
        $('.btn-cancel').click(function() {
            ...
            FoxUI.confirm(lang_js_biz_order_refund_5+'?', '', function() {
                $(this).attr('stop', 1).attr('buttontext', $(this).html()).html(lang_js_biz_order_refund_6+'..');
        ...
        $('#express_submit').click(function() {
            ...
            if ($('#expresssn').isEmpty()) {
                FoxUI.toast.show(lang_js_biz_order_refund_7);
                return
            }
            $(this).html(lang_js_biz_order_refund_8+'...').attr('stop', 1);
            core.json('order/refund/express', {
                ...
                if (postjson.status == 1) {
                    ...
                    $('#express_submit').html(lang_js_biz_order_refund_9).removeAttr('stop');
                    FoxUI.toast.show(postjson.result.message)
                }
            }, true, true)
        });
        $('.btn-receive').click(function() {
            ...
            FoxUI.confirm(lang_js_biz_order_refund_10+'?', '', function() {
                $(this).attr('stop', 1).html(lang_js_biz_order_refund_11+'...');
                core.json('order/refund/receive', {
                    ...
                    if (postjson.status == 1) {
                        ...
                        $('.btn-receive').html(lang_js_biz_order_refund_12).removeAttr('stop');
                        FoxUI.toast.show(postjson.result.message)
                    }
                }, true, true)
            })
