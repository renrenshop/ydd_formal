define(['core', 'tpl'], function(core, tpl) {
    ...
    modal.init = function(fromDetail) {
        ...
        $('.order-cancel select').unbind('change').change(function() {
            ...
            FoxUI.confirm(lang_js_biz_order_op_0+'?', lang_js_biz_order_op_1, function() {
                modal.cancel(orderid, val, true)
            })
        });
        $('.order-delete').unbind('click').click(function() {
            var orderid = $(this).data('orderid');
            FoxUI.confirm(lang_js_biz_order_op_2+'?',lang_js_biz_order_op_3, function() {
                modal.delete(orderid, 1)
            })
        });
        $('.order-deleted').unbind('click').click(function() {
            var orderid = $(this).data('orderid');
            FoxUI.confirm(lang_js_biz_order_op_4+'?', lang_js_biz_order_op_5, function() {
                modal.delete(orderid, 2)
            })
        });
        $('.order-recover').unbind('click').click(function() {
            var orderid = $(this).data('orderid');
            FoxUI.confirm(lang_js_biz_order_op_6+'?', lang_js_biz_order_op_7, function() {
                modal.delete(orderid, 0)
            })
        });
        $('.order-finish').unbind('click').click(function() {
            var orderid = $(this).data('orderid');
            FoxUI.confirm(lang_js_biz_order_op_8+'?', lang_js_biz_order_op_9, function() {
    ...
    modal.verify = function(orderid, verifycode) {
        ...
        core.json('verify/qrcode', {
            ...
            if (ret.status == 0) {
                FoxUI.alert(lang_js_biz_order_op_10+'，'+lang_js_biz_order_op_11+'!');
                return
            }
            var time = +new Date();
