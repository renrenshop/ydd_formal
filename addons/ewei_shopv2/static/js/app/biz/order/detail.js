define(['core', 'tpl', 'biz/order/op'], function(core, tpl, op) {
    ...
    modal.init = function(params) {
        ...
        $('.btn-cancel').click(function() {
            ...
            FoxUI.confirm(lang_js_biz_order_detail_0+'?', '', function() {
                $(this).attr('stop', 1).attr('buttontext', $(this).html()).html(lang_js_biz_order_detail_1+'..');
                ...
                if (this.getStatus() == BMAP_STATUS_SUCCESS) {
                    ...
                    $('.store-item').each(function() {
                        ...
                        if (store_lng > 0 && store_lat > 0) {
                            ...
                            location.html(lang_js_biz_order_detail_2+': ' + distance.toFixed(2) + "km").show()
                        } else {
                            $(this).data('distance', 999999999999999999);
                            location.html(lang_js_biz_order_detail_3).show()
                        }
                        arr.push($(this))
                    });
                    ...
                    $('#nearStoreHtml').find('.location').html(location + "<span class='fui-label fui-label-danger'>"+lang_js_biz_order_detail_4+"</span> ");
                    $(arr[0]).remove()
                }
            }, {
