define(['core', 'tpl'], function(core, tpl) {
    ...
    modal.changeMode = function() {
        ...
        if (modal.status == 'favorite') {
            ...
            $('.editmode').show();
            modal.status = 'edit';
            $('.btn-edit').html(lang_js_biz_member_favorite_0);
            $('.favoritecover').show()
        } else {
            $('.fui-content').removeClass('navbar');
            $('.editmode').hide();
            modal.status = 'favorite';
            $('.btn-edit').html(lang_js_biz_member_favorite_1);
            $('.favoritecover').hide()
        }
    };
    ...
    modal.toggle = function(goodsid, isfavorite, callback) {
        ...
        core.json(url, {
            ...
            if (ret.status == 0) {
                if (ret.result.message == lang_js_biz_goods_picker_8+'!') {
    ...
    modal.remove = function(ids, callback) {
        ...
        FoxUI.confirm(lang_js_biz_member_favorite_2+'?', function() {
