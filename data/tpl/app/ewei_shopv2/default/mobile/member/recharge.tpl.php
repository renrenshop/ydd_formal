...
<?php if (file_exists('/www/wwwroot/fxv2_d/addons/ewei_shopv2/lang/'.$_W['lang_type'].'/_template_mobile_member_recharge.php')){include('/www/wwwroot/fxv2_d/addons/ewei_shopv2/lang/'.$_W['lang_type'].'/_template_mobile_member_recharge.php');}?>
...
<div class='fui-page fui-page-current'>
		...
		<?php  if($wechat['success'] || $payinfo['wechat'] || ($alipay['success'] && !is_h5app()) || $payinfo['alipay']) { ?>
			...
			<div class='fui-cell-group'>
				<div class='fui-cell'>
					...
					<div class='fui-cell-info c000'>RM <?php  echo number_format($credit,2)?></div>
				</div>
				<div class='fui-cell'>
					<div class='fui-cell-label'>充值金额</div>
					<div class='fui-cell-info c000'style="    display: -webkit-box;display: -webkit-flex;display: -ms-flexbox;display: flex;-webkit-box-align: center;-webkit-align-items: center;-ms-flex-align: center;align-items: center;">RM&nbsp;<input type='number' class='fui-input' id='money' value="<?php  echo $_GPC['money'];?>"></div>
				</div>
			</div>
			<div class='fui-cell-group'>
				<?php  if(com('coupon')) { ?>
			...
			<div class='fui-cell-group'>
				...
				<?php  if($rmpay['success']) { ?>
				<div class="fui-cell applyradio " id="btn-rm1">
					<div class="fui-cell-icon"><img src="<?php echo EWEI_SHOPV2_STATIC;?>images/pay/rm.png" alt=""></div>
					<div class="fui-cell-info" style="height: 100%">
						<div class="title">RmPay充值</div>
						<div class="subtitle" style="color: #999999;font-size: .65rem">RmPay安全支付</div>
					</div>
					<div class="fui-cell-remark noremark"><input name="1" type="radio" class="fui-radio fui-radio-danger" id="applytype3" data-type="0" checked="cheched"></div></label >
				</div>
				<?php  } ?>
			</div>
			...
			<?php  } ?>
			<?php  if($rmpay['success']) { ?>
				<a id='btn-rm' class='btn btn-danger mtop  block btn-pay'  style='display:none'>确认支付</a>
			<?php  } ?>
		<?php  } else { ?>
	...
	<script language='javascript'>
        require(['biz/member/recharge'], function (modal) {
            modal.init({minimumcharge: <?php  echo $minimumcharge?>,rmpay: <?php  echo intval($rmpay['success'])?>,wechat: <?php  echo intval($wechat['success'])?>,alipay:<?php  echo intval($alipay['success'])?>});
        });
	</script>
	<script>
        $(function(){
